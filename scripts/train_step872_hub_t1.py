"""Step 872: Hub aggregation T1 — does alpha=0.05 advantage hold at calibration?

MOTIVATION
==========
step869 T0 (20ep, 50% data, 5060ti_cuda):
  Ref_dw    : 93.91%
  A_hub005  : 94.24% (+0.33pp ADVANCES)  alpha_hub=0.05 — very light global mean
  B_hub03   : 93.27% (-0.64pp KILLED)    larger alpha washes out local structure
  C_hub10   : 88.00% (-5.91pp KILLED)    catastrophic at full hub
  D_hub_beam: 92.61% (-1.30pp KILLED)    beam-gated hub also fails

alpha=0.05 is the sweet spot: just enough global context to help, not enough to
homogenize representations. This matches intuition — the global mean provides
a weak "background signal" about aggregate network state.

CONFIGS (T1: 75ep, 50% data, seed=42, 5060ti_cuda)
  Ref_dw   : alpha_hub=0.0 — standard ΔW-proj
  A_hub005 : alpha_hub=0.05 — advancing config from T0

SUCCESS: A_hub005 ≥ Ref_dw + 0.2pp → hub advances to T2 and defaults
VIABLE:  A_hub005 ≥ Ref_dw + 0.0pp → hub advances to T2
KILL:    A_hub005 < Ref_dw − 0.1pp → T0 was noise
"""
...
OUT_PATH = ROOT / "results" / f"train_step872_hub_t1_seed{SEED}__{SLOT}.json"

STEP869_HUB005_T0 = 0.9424   # A_hub005 T0 (step869)
...
CONFIGS = {
    "Ref_dw":   (0.0,  "alpha_hub=0.0 — standard ΔW-proj (T1 baseline)"),
    "A_hub005": (0.05, "alpha_hub=0.05 — very light global mean (T0 winner)"),
}
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"step872 — Hub aggregation T1 (75ep, 50% data)")
    print(f"  device={DEVICE}  epochs={EPOCHS}  seed={SEED}")
    print(f"  T0 ref (step869): Ref_dw={STEP869_HUB005_T0:.4f} (alpha=0.05)")
    print(f"  T1 ref (step858): ΔW-proj T1={STEP858_DW_T1:.4f}")
    ...
    print(f"\n{'='*70}")
    print(f"STEP 872 SUMMARY — Hub T1")
    print(f"{'='*70}")
    ...
    if hub and ref:
        ...
        if gap >= 0.002:
            verdict = "STRONG — hub advances to T2, add to base"
        elif gap >= 0.0:
            verdict = "VIABLE — hub advances to T2"
        else:
            verdict = "KILL — T0 was noise"
        print(f"\n  Hub T1 verdict: {gap*100:+.2f}pp → {verdict}")
    print(f"\n-> {OUT_PATH}")
