"""Step 265: K_iter=4 ΔW proj at N=4096 TIER-2 (150ep 100% data).

MOTIVATION
==========
step263 T1 seed=42: K=5=95.01%, K=4=95.97% → K=4 wins +0.97pp at N=4096.
This is a STRONG signal at T1. If it holds at T2, K=4 becomes the new D=16 ceiling config.

Prior D=16 ceiling (K=5 T2): step205 = 97.17% @ep118 at N=4096.

Target: K=4 T2 ≥ 97.17% (step205). If yes, K=4 is the new efficiency-plus-ceiling config.

CONFIGS (N=4096 D=16 K_hh=2 ΔW proj, seed=42, 150ep 100% data)
  Ref : K_iter=5 (ΔW proj)  -- comparison vs step205 = 97.17% AH-only
  A   : K_iter=4 (ΔW proj)  -- candidate winner
"""
...
OUT_PATH = ROOT / "results" / f"train_step265_kiter4_n4096_tier2_seed{SEED}.json"

CONFIGS = [
    dict(key="Ref", K_iter=5, label="ΔW proj K_iter=5 N=4096 (T2)"),
    dict(key="A",   K_iter=4, label="ΔW proj K_iter=4 N=4096 (T2 — candidate winner)"),
]
...
def main():
    ...
    print(f"Step 265 — K_iter=4 ΔW proj at N={N} TIER-2 (150ep 100% data)")
    print(f"  seed={SEED}  EPOCHS={EPOCHS}  device={DEVICE}")
    print(f"  Target: K=4 T2 ≥ 97.17% (step205 K=5 AH-only T2)")
    ...
    print(f"\n\n========== STEP 265 SUMMARY (N={N} T2 seed={SEED}) ==========")
    for cfg in active:
