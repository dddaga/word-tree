"""Step 756: K_iter=3 at N=8192 K_hh=2 T1 + inline inference latency benchmark.

K_hh=2 FIXED (vs step751 which uses K_hh=round(N/1000)=8).
PURPOSE: test whether K_hh=2 is universally good (efficiency config) or needs to
scale with N. Paired comparison with step751 (same N, K_iter; only K_hh differs).

NOTE: step211 already ran N=8192 K_hh=2 K_iter=3 at T1, achieving 94.93% (borderline).
Step756 REPRODUCES that config with the inline latency benchmark added.
The training result should land near 94.93%. If it deviates >1pp, flag as seed variance.

Existing curve:
  N=8192 K_hh=8 K_iter=3: step751 (K_hh=round(N/1000) rule)
  N=8192 K_hh=2 K_iter=3: 94.93% (step211 T1, no latency bench) → THIS STEP adds bench
...
D = 16; K_HH = 2; K_IN = 25; K_ITER = 3   # K_hh=2 FIXED (efficiency config value)
ALPHA_REFLECT = 0.5; ALPHA_TURING = 0.0
FLOPS = 3 * N * K_HH * D * K_ITER
OUT_PATH = ROOT / "results" / "train_step756_kiter3_n8192_khh2.json"
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 756 — K_iter=3 @ N=8192 K_hh=2 (fixed) T1 + inline latency")
    print(f"N={N} D={D} K_hh={K_HH} K_iter={K_ITER} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"K_hh=2 FIXED. Reproduces step211 (94.93%) + adds latency bench.")
    print(f"Paired with step751 (K_hh=8). Threshold: ≥96%")
    print(f"{'='*70}")
    ...
    if abs(best - 0.9493) > 0.01:
        print(f"  NOTE: >1pp deviation from step211 (94.93%) — flag as seed variance")
