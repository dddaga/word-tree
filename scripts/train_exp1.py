"""Exp 1 (Stage B) training: GA search then full training.

Stage B = spatial path-length phase only (use_proximity=True, use_wphase=False).
Compared against Stage A (static binary C) to measure proximity routing benefit.

Output:
  results/exp1_ga_results.json  -- best hyperparams from GA search
  results/exp1_full.json        -- full training metrics (top1, mAP, per-class)
  checkpoints/exp1_best.pt      -- best model checkpoint
"""
...
def run_ga_search(
    ...
    """Run GA hyperparameter search for Exp 1 (Stage B)."""
    ga = GASearch(
    ...
    with open("results/exp1_ga_results.json", "w") as f:
...
def run_full_training(best: dict, data: tuple, device: str) -> tuple:
    """Train SGNNET_Wave Exp 1 with best GA config."""
    ...
    model = SGNNET_Wave(
        ...
        use_proximity=True, use_wphase=False,
    )
    ...
    torch.save(model.state_dict(), "checkpoints/exp1_best.pt")
...
def evaluate_and_save(model, trainer, history, best: dict) -> dict:
    ...
    with open("results/exp1_full.json", "w") as f:
        json.dump(result, f, indent=2)

    print(
        f"Exp 1: top1={metrics['top1_accuracy']:.4f}, "
