"""Step 715: ΔW projection at N=128 — extending headroom curve.

Tier-2 curve: N=2048(+1.56pp) N=1024(+4.68pp) N=512(+10.96pp) N=256(+20.02pp) N=128(?)
FLOPs=61,440 (~0.06M) — 6% of original 0.98M record.
Running Tier-1 (75ep, 50% data) to establish the gain at minimum scale.
"""
...
OUT_PATH = ROOT / "results" / "train_step715_delta_n128.json"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Step 715 — ΔW proj N=128 Tier-1 (headroom curve extension)")
    print(f"N={N} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"Curve T2: N=2048(+1.56) N=1024(+4.68) N=512(+10.96) N=256(+20.02) N=128(?)")
    print(f"{'='*70}")
    ...
    if "Ref" in results and "A_proj" in results:
        print(f"\nRef={ref:.4f}  A_proj={proj:.4f}  Δ={proj-ref:+.4f}")
        print(f"Curve: N=256(+20.02) N=512(+10.96) N=1024(+4.68) N=2048(+1.56) → N=128({proj-ref:+.2f})")
