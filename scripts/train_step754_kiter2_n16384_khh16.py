"""Step 754: K_iter=2 at N=16384 K_hh=16 T1 + inline inference latency benchmark.

Phase 2 scale — test whether more nodes at K_iter=2 recovers accuracy lost
from reducing K_iter in the 8192-node experiments.

K_hh = round(N/1000) = 16.

Existing curve:
  N=8192  K_hh=8  K_iter=2: step752
  N=16384 K_hh=16 K_iter=2: THIS STEP
...
parser = argparse.ArgumentParser(
    description="Step 754: K_iter=2 @ N=16384 K_hh=16 T1 + inline latency bench"
)
...
D = 16; K_HH = 16; K_IN = 25; K_ITER = 2
ALPHA_REFLECT = 0.5; ALPHA_TURING = 0.0
FLOPS = 3 * N * K_HH * D * K_ITER
OUT_PATH = ROOT / "results" / "train_step754_kiter2_n16384_khh16.json"
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 754 — K_iter=2 @ N=16384 K_hh=16 T1 + inline latency")
    print(f"N={N} D={D} K_hh={K_HH} K_iter={K_ITER} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"Threshold: ≥96%  (scale-up to recover K_iter=2 accuracy)")
    print(f"{'='*70}")
    ...
    if best >= 0.96:
        print(f"  → K_iter=2 @ N=16384 is Pareto-viable; add to latency-accuracy plot")
    else:
        print(f"  → N=16384 K_iter=2 does not recover accuracy; K_iter floor is binding")
