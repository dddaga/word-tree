"""Step 281: K_in=15 + aug compound @ N=8192 (T1).

MOTIVATION
==========
Compound (K_in=15+aug) validated at:
  N=2048 T2: 95.46% (+0.18pp vs ref, 26.7× seed reduction) — step274
  N=4096 T2: running — step279 (expected ~97.4%)

Question: does the compound hold at N=8192? If so, K_in=15+aug delivers
~N=8192 accuracy with 26.7× seed FLOP reduction across the full scaling curve.

N=8192 no-aug T2 = 97.17% (step209). N=8192+aug T2 = 97.38% (step276).
If K_in=15 costs −0.33pp (consistent across N=2048/4096), compound ~97.05%.

CONFIGS
=======
  Ref_n8192       : N=8192, K_in=25, no-aug (step209 ref ~97.17%)
  C_k15_aug_n8192 : N=8192, K_in=15, aug    (COMPOUND — T1 filter)
...
parser.add_argument("--configs",  default="Ref_n8192,C_k15_aug_n8192")
...
OUT_PATH = ROOT / "results" / f"train_step281_kin15_aug_n8192_t1_seed{SEED}__{SLOT}.json"

# (K_in, use_aug, description)
CONFIGS = {
    "Ref_n8192":       (25, False, "N=8192 K_in=25 no-aug (step209 ref)"),
    "C_k15_aug_n8192": (15, True,  "N=8192 K_in=15 + aug (COMPOUND T1)"),
}
...
def main():
    ...
    print(f"Step 281 — K_in=15+aug compound @ N=8192 (T1: {EPOCHS}ep, 50% data)")
    ...
    print(f"\n{'='*60}")
    print(f"STEP 281 SUMMARY — K_in=15+aug compound @ N=8192 (T1)")
    ...
    for k in keys:
        ...
        print(f"  {k:<24} K_in={r['K_in']} aug={'y' if r['use_aug'] else 'n'}  "
