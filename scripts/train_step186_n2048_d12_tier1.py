"""Step 186: N=2048 D=12 K_hh=4 scratch Tier-1 — sub-2.4M FLOPs floor probe.

MOTIVATION
==========
FLOPs frontier confirmed phase exits:
  step181: 96.03% @ ~3.93M FLOPs (N=2048 D=20 Tier-2) ← current min record
  step185: ~94.8% @ ~3.15M FLOPs (N=2048 D=16 Tier-2, ~20ep left, likely no phase exit)

If step185 doesn't hit 95%, floor stays at 3.93M. This probes further below:
N=2048 D=12 K_hh=4: FLOPs = 3×2048×4×12×8 = 2,359,296 ≈ 2.36M (62% below budget)

Pattern so far: D=16 Tier-1=93.96% → Tier-2 peaked 94.83% (short of 95%).
D=12 Tier-1 likely ~91-93%. If ≥91%, Tier-2 may still hit 95%.

Phase-exit criterion: ≥95% @ ≤6.18M FLOPs. ~2.36M ✓ (62% below budget).

CONFIGS (N=2048, D=12, K_hh=4, K_iter=8, AH=1.0, 50% data, 75ep — Tier-1)
"""
...
OUT_PATH = ROOT / "results" / "train_step186_n2048_d12_tier1.json"


def main():
    print(f"\n{'='*70}")
    print(f"Step 186 — N=2048 D=12 K_hh=4 scratch Tier-1 (50% data 75ep)")
    print(f"FLOPs={FLOPS:,} (~{FLOPS/1e6:.2f}M) — 62% BELOW 6.18M budget")
    print(f"step181 ref: 96.03% @ ~3.93M | step185 (D=16): ~94.8% @ ~3.15M (no exit)")
    print(f"{'='*70}")
    ...
    result = {"A": {"N": N, "D": D, "K_hh": K_HH, "K_iter": K_ITER,
                    ...
                    "label": "A scratch N=2048 D=12 K_hh=4 α=1.0 50% 75ep"}}
    OUT_PATH.parent.mkdir(exist_ok=True)
    OUT_PATH.write_text(json.dumps(result, indent=2))
    print(f"\n{'='*70}")
    print(f"STEP 186: {best:.4f}  vs_step182={best-0.9396:+.4f}  FLOPs={FLOPS/1e6:.2f}M")
