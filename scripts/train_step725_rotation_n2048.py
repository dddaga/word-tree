"""Step 725: ΔW rotation vs projection at N=2048 Tier-1 — formal crossover confirmation.

proj vs rot monotone advantage curve (all Tier-1):
  N=256:  proj +19.13pp vs rot +7.89pp  → proj wins +11.24pp (step722)
  N=512:  proj +12.64pp vs rot +5.79pp  → proj wins +6.85pp  (step721)
  N=1024: proj +4.87pp  vs rot +2.96pp  → proj wins +1.91pp  (step723)
  N=2048: ??? ← this step (Tier-1)
  N=2048: rot +2.52pp vs proj +2.11pp   → rot wins +0.41pp  (step301, Tier-0 only)

Crossover is between N=1024 and N=2048. This step pins the Tier-1 quality result
at the efficiency config.

Configs:
  Ref    : AH α=1.0 (efficiency baseline)
  A_proj : ΔW projection
  A_rot  : ΔW rotation (learned angle in (Z_nb, ΔW) plane)
"""
...
OUT_PATH = ROOT / "results" / "train_step725_rotation_n2048.json"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Step 725 — ΔW rot vs proj N=2048 Tier-1 (crossover confirmation)")
    print(f"N={N} D={D} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"Curve: N=256(proj+11.24) N=512(proj+6.85) N=1024(proj+1.91) N=2048(?)")
    print(f"{'='*70}")
    ...
    if not math.isnan(d_rot) and not math.isnan(d_proj):
        ...
        print(f"  Winner: {winner} by {margin:.2f}pp")
        print(f"  Crossover curve: N=256(proj+11.2) N=512(proj+6.9) N=1024(proj+1.9) N=2048({winner[2:]}+{margin:.1f})")
