"""Step 729: ΔW rotation vs projection at N=4096 Tier-1 confirmation.

step728 T0 result (both devices):
  mini:  Ref=94.50%  A_proj=94.14%(−0.36pp)  A_rot=94.62%(+0.12pp)
  studio: Ref=94.52%  A_proj=94.37%(−0.15pp)  A_rot=94.55%(+0.03pp)

T0 verdict: rot marginally positive (+0.03–0.12pp) while proj hurts (−0.15 to −0.36pp).
But rot's T0 gain is within noise for T0 (ρ=0.80 predictor). T1 needed to confirm.

QUESTION: Is rot's marginal ceiling advantage real or T0 noise?
  If rot > ref at T1 by ≥+0.5pp → rot is a real mechanism at ceiling (use for N=4096 paper)
  If rot ≈ ref at T1 → neither mechanism helps at ceiling (both N-specific, proj hurts, rot neutral)

Paper consequence:
  → proj = low-N mechanism (+12–19pp at N=256–512, diminishing to 0 at N=2048+)
  → rot = marginally better at ceiling but economically uninteresting (3× heavier, tiny gain)
...
OUT_PATH = ROOT / "results" / "train_step729_rotation_n4096_t1.json"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Step 729 — ΔW rot vs proj N=4096 Tier-1 (ceiling confirmation)")
    print(f"N={N} D={D} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"T0: proj=−0.15 to −0.36pp (hurts); rot=+0.03 to +0.12pp (marginal)")
    print(f"QUESTION: Is rot's marginal ceiling advantage real at T1?")
    print(f"{'='*70}")
    ...
    if not math.isnan(d_rot) and not math.isnan(d_proj):
        if d_rot >= 0.005 and d_proj < 0:
            print(f"  → ROT CONFIRMED AT CEILING: rot real gain ({d_rot*100:+.2f}pp), proj hurts ({d_proj*100:+.2f}pp)")
            print(f"  → Paper: rotation = near-ceiling mechanism (3× heavier but sole survivor)")
        elif abs(d_rot) < 0.005:
            print(f"  → NEUTRAL: rot ≈ ref at ceiling (T0 noise confirmed). Neither mechanism helps N=4096.")
            print(f"  → Paper: both ΔW mechanisms are N-specific; neither generalizes to D=16 ceiling")
        else:
            print(f"  → Unexpected result: proj={d_proj:+.4f} rot={d_rot:+.4f}")
