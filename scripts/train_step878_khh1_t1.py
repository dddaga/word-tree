"""Step 878: K_hh=1 T1 calibration — paper efficiency claim.

MOTIVATION
==========
step865 T0 (20ep, 50% data): K_hh=1 = 93.25% (-0.48pp vs Ref 93.73%) — VIABLE.
K_hh=1 (K_local=0, K_random=1) uses 50% fewer routing MACs:
  K_hh=2: 2 × N × K_iter × D × 2 ops
  K_hh=1: 1 × N × K_iter × D × 2 ops → 50% routing MAC reduction

PAPER CLAIM: If T1 delta ≥ -1.0pp, we can claim half-routing at negligible
accuracy cost. This is a strong efficiency story complementary to K=1 student.
...
SUCCESS: A_khh1 ≥ Ref - 0.5pp → strong efficiency claim (half routing, <0.5pp)
VIABLE:  A_khh1 ≥ Ref - 1.0pp → viable efficiency claim (half routing, <1pp)
KILL:    A_khh1 < Ref - 1.0pp → K_hh=2 confirmed minimum viable routing density
"""
...
OUT_PATH = ROOT / "results" / f"train_step878_khh1_t1_seed{SEED}__{SLOT}.json"

STEP865_T0_REF = 0.9373   # Ref_dw T0
STEP865_T0_A   = 0.9325   # A_khh1 T0 (-0.48pp VIABLE)
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"step878 — K_hh=1 T1 calibration (75ep, 50% data)")
    print(f"  device={DEVICE}  epochs={EPOCHS}  seed={SEED}")
    print(f"  T0 results: Ref={STEP865_T0_REF:.4f}, A_khh1={STEP865_T0_A:.4f} (-0.48pp VIABLE)")
    ...
    for key in keys:
        ...
        delta = best - (ref_acc if ref_acc is not None else STEP865_T0_REF)
    ...
    print(f"\n{'='*70}")
    print(f"STEP 878 SUMMARY — K_hh=1 T1")
    print(f"{'='*70}")
    ...
    if khh1 and ref:
        ...
        if gap >= -0.005:
            verdict = "STRONG — K_hh=1 is a paper efficiency finding (<0.5pp, 50% routing)"
        elif gap >= -0.01:
            verdict = "VIABLE — K_hh=1 efficiency claim holds (<1pp, 50% routing)"
        else:
            verdict = "KILL — K_hh=1 fails at T1, K_hh=2 confirmed minimum"
        print(f"\n  K_hh=1 T1: {gap*100:+.2f}pp → {verdict}")
    print(f"\n-> {OUT_PATH}")
