"""CNN distillation step035 — GA5 C=(48,96,384) k=5 T1 calibration.
T0 (step034): 66.39% @ep20 — not clearly failing; advanced per T0 rejection-filter rule.
T0 threshold comparison was wrong (vs T1 baseline); real T0 gap vs k=3 estimated 2-4pp.
Hypothesis: larger dw_kernel improves spatial aggregation. ConvNeXt default=7; Ref/D_small_s k=7 STRONG.
Baseline: GA5 k=3 T1=74.73% (step029). Advance>=75.23% (+0.50pp).
531K params, 146.1M MACs (vs k=3: 523K, 141.3M). T1: 75ep 50%data seed=42.
"""
...
OUT = ROOT / "results" / f"cnn_step035_ga5k5_t1__{SLOT}.json"

GA5_CH, GA5_K, GA5_EXP, GA5_SIDE, GA5_CRELU = (48, 96, 384), 5, 2, False, True
GA5_K3_T1 = 0.7473   # step029 GA5 k=3 T1 baseline
ADVANCE = 0.7523      # +0.50pp over k=3 T1
...
def main():
    ...
    print(f"cnn_step035 GA5 k=5 T1 | {args.epochs}ep 50%data | alpha={args.mixup_alpha}")
    print(f"  {DEVICE}  slot={SLOT}  params={n_p:,}  MACs={macs:.1f}M")
    print(f"  Baseline={GA5_K3_T1:.4f} (GA5 k=3 T1). Advance>={ADVANCE:.4f}")
    ...
    print(f"\n{'='*60}\ncnn_step035 RESULT — GA5 k=5 T1\n{'='*60}")
    print(f"  best={best:.4f} @ep{best_ep}  Δ_GA5k3={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    adv = "ADVANCE → T2 GA5 k=5 (step036)" if verdict == "ADVANCE" else "NO-GAIN → k=3 optimal; larger kernel no benefit at GA5 scale"
    print(f"  -> {adv}")
