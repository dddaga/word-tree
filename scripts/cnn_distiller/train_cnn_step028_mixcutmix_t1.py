"""CNN distillation step028 — MixCutMix: alternating Mixup/CutMix, GA2.
Question: Does stochastic switching between Mixup and CutMix per batch improve
          over either alone? Last cheap regularization test before arch pivot.
Context: Regularization ceiling found at T1:
           Mixup α=0.2: 73.61% (baseline), CutMix α=0.2: 73.81% (+0.20pp).
           All methods within ±0.63pp — architecture appears to be bottleneck.
Hypothesis: Alternating Mixup/CutMix covers both global (pixel-blend) and local
            (patch) data space per epoch, complementary regularization diversity.
            50% batch → Mixup, 50% → CutMix, same α=0.2.
Loss: unchanged — 0.50·feat_cos + 0.30·dkd + 0.20·ce.
Baseline: step021 GA2+Mixup T1 = 73.61%. Advance: ≥74.11% (+0.5pp).
If NO-GAIN: regularization direction closed; pivot to architecture (GA5 larger C).
Output: results/cnn_step028_mixcutmix_t1__{SLOT}.json
"""
...
OUT = ROOT / "results" / f"cnn_step028_mixcutmix_t1__{SLOT}.json"
...
def augment_batch(img, feat, soft, lbl, alpha):
    lam_raw = float(np.random.beta(alpha, alpha))
    idx = torch.randperm(img.size(0), device=img.device)
    if np.random.rand() < 0.5:  # Mixup
        img_out = lam_raw * img + (1 - lam_raw) * img[idx]
        feat_out = lam_raw * feat + (1 - lam_raw) * feat[idx]
        soft_out = lam_raw * soft + (1 - lam_raw) * soft[idx]
        lam_out = lam_raw
    else:  # CutMix
        H, W = img.shape[2], img.shape[3]
        x1, y1, x2, y2 = rand_bbox(H, W, lam_raw)
        lam_out = 1.0 - (x2 - x1) * (y2 - y1) / (H * W)
        img_out = img.clone()
        img_out[:, :, y1:y2, x1:x2] = img[idx, :, y1:y2, x1:x2]
        feat_out = lam_out * feat + (1 - lam_out) * feat[idx]
        soft_out = lam_out * soft + (1 - lam_out) * soft[idx]
    lam_t = torch.full((img.size(0),), lam_out, device=img.device)
    return img_out, feat_out, soft_out, lbl, lbl[idx], lam_t
...
def main():
    ...
    print(f"cnn_step028 GA2+MixCutMix(α={args.alpha}) T1 | {args.epochs}ep 50%data | seed={args.seed}")
    print(f"GA#2: C=(32,64,384) k=3 exp=2 crelu  params={n_p:,}  MACs={macs:.1f}M")
    print(f"Each batch: 50% Mixup, 50% CutMix. Loss: 0.50·feat + 0.30·dkd + 0.20·ce")
    print(f"Baseline (step021 GA2+Mixup): {STEP021_BEST:.4f}  Advance>={ADVANCE:.4f}")
    ...
    for ep in range(args.epochs):
        ...
        for img, tf_, soft, lbl in tr:
            ...
            img, tf_, soft, lbl_a, lbl_b, lam = augment_batch(img, tf_, soft, lbl, args.alpha)
            logits, sf = model(img)
    ...
    print(f"\n{'='*60}\ncnn_step028 RESULT — GA2+MixCutMix(α={args.alpha}) T1\n{'='*60}")
    print(f"  best={best:.4f} @ep{best_ep}  Δ_step021={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    adv_msg = "ADVANCE to T2 multi-seed (step029)" if verdict == "ADVANCE" else "NO-GAIN — regularization ceiling reached; pivot to arch (GA5)"
    print(f"  -> {adv_msg}")
    OUT.parent.mkdir(exist_ok=True)
    OUT.write_text(json.dumps({"best": best, "best_ep": best_ep, "delta": delta,
                               "verdict": verdict, "elapsed": elapsed, "alpha": args.alpha,
