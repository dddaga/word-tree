"""CNN distillation step031 — GA5 C=(48,96,384) + CutMix(α=0.2) T1 scout.
Hypothesis: CutMix gave +0.20pp vs Mixup on GA2 T1 (step027 vs step021).
If same delta holds at GA5 scale, expect ~74.93%+ (GA5+Mixup T1=74.73%).
Config: GA5 C=(48,96,384) k=3 exp=2 crelu=True side=False cutmix_alpha=0.2
Loss: 0.50·feat_cos + 0.30·dkd + 0.20·ce (unchanged from GA5+Mixup)
Baseline: GA5+Mixup T1=74.73% (step029). Advance>=75.23% (+0.5pp).
Reference: GA2+CutMix T1=73.81% (step027, +0.20pp vs GA2+Mixup 73.61%).
"""
...
OUT = ROOT / "results" / f"cnn_step031_ga5_cutmix_t1__{SLOT}.json"

GA5_CH, GA5_K, GA5_EXP, GA5_SIDE, GA5_CRELU = (48, 96, 384), 3, 2, False, True
BASELINE = 0.7473   # GA5+Mixup T1 (step029)
ADVANCE  = 0.7523   # +0.50pp
GA2_CUTMIX_T1 = 0.7381  # step027 reference
...
def cutmix_batch(img, feat, soft, lbl, alpha):
    ...
    img_cut = img.clone()
    img_cut[:, :, x1:x2, y1:y2] = img[idx, :, x1:x2, y1:y2]
    lam_adj = 1.0 - (x2 - x1) * (y2 - y1) / (img.size(2) * img.size(3))
    feat_mix = lam_adj * feat + (1 - lam_adj) * feat[idx]
    soft_mix = lam_adj * soft + (1 - lam_adj) * soft[idx]
    return (img_cut, feat_mix, soft_mix, lbl, lbl[idx],
            torch.full((img.size(0),), lam_adj, device=img.device))
...
def main():
    ...
    print(f"cnn_step031 GA5+CutMix T1 | {args.epochs}ep 50%data | alpha={args.cutmix_alpha}")
    print(f"  {DEVICE}  slot={SLOT}  params={n_p:,}  MACs={macs:.1f}M")
    print(f"  Baseline={BASELINE:.4f} (GA5+Mixup T1)  Advance>={ADVANCE:.4f}")
    print(f"  GA2+CutMix T1 ref={GA2_CUTMIX_T1:.4f} (step027, +0.20pp vs GA2+Mixup)")
    ...
    print(f"\n{'='*60}\ncnn_step031 RESULT — GA5+CutMix T1\n{'='*60}")
    print(f"  best={best:.4f} @ep{best_ep}  Δ_GA5mix={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    adv = "ADVANCE → T2 multi-seed GA5+CutMix (step032)" if verdict == "ADVANCE" else "NO-GAIN → CutMix no improvement over Mixup at GA5 scale"
    print(f"  -> {adv}")
    OUT.parent.mkdir(exist_ok=True)
    OUT.write_text(json.dumps({"best": best, "best_ep": best_ep, "baseline": BASELINE,
                               "delta": delta, "verdict": verdict, "elapsed": elapsed,
                               "cutmix_alpha": args.cutmix_alpha, "channels": list(GA5_CH),
