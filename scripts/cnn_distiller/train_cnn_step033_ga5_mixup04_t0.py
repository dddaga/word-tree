"""CNN distillation step033 — GA5 C=(48,96,384) + Mixup(α=0.4) T0 scout.
Hypothesis: GA5 wider model (523K vs GA2 481K) has more capacity → benefits from
stronger regularization. α=0.4 doubles interpolation strength vs α=0.2.
Baseline: GA5+Mixup(α=0.2) T1=74.73% (step029). Advance>=75.23% (+0.5pp).
Config: GA5 C=(48,96,384) k=3 exp=2 crelu=True side=False mixup_alpha=0.4
Loss: 0.50·feat_cos + 0.30·dkd + 0.20·ce (unchanged)
T0: 20ep 50%data (rejection filter)
"""
...
OUT = ROOT / "results" / f"cnn_step033_ga5_mixup04_t0__{SLOT}.json"

GA5_CH, GA5_K, GA5_EXP, GA5_SIDE, GA5_CRELU = (48, 96, 384), 3, 2, False, True
GA5_T1_ALPHA02 = 0.7473   # step029 baseline
...
def main():
    ...
    print(f"cnn_step033 GA5+Mixup(α={args.mixup_alpha}) T0 | {args.epochs}ep 50%data")
    print(f"  {DEVICE}  slot={SLOT}  params={n_p:,}  MACs={macs:.1f}M")
    print(f"  Baseline={GA5_T1_ALPHA02:.4f} (GA5+Mixup α=0.2 T1)  Advance>={ADVANCE:.4f}")
    ...
    delta = best - GA5_T1_ALPHA02
    verdict = "ADVANCE" if best >= ADVANCE else "NO-GAIN"
    print(f"\n{'='*60}\ncnn_step033 RESULT — GA5+Mixup(α={args.mixup_alpha}) T0\n{'='*60}")
    print(f"  best={best:.4f} @ep{best_ep}  Δ_GA5_α02={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    adv = "ADVANCE → T1 GA5+Mixup(α=0.4) (step034)" if verdict == "ADVANCE" else "NO-GAIN → α=0.4 over-regularizes"
    print(f"  -> {adv}")
    OUT.parent.mkdir(exist_ok=True)
    OUT.write_text(json.dumps({"best": best, "best_ep": best_ep, "baseline": GA5_T1_ALPHA02,
