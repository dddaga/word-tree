"""CNN distillation step029 — GA5: C=(48,96,384) T1 scout.
Arch pivot: regularization CLOSED (steps021-028, all ±0.60pp). C1/C2 width is bottleneck.
GA5: 50% wider C1+C2 vs GA2. Loss/Mixup unchanged. Stays under Ref on params+MACs.
Baseline: step021 GA2 T1=73.61%. Advance>=74.11%.
"""
...
OUT = ROOT / "results" / f"cnn_step029_ga5_t1__{SLOT}.json"
...
def main():
    ...
    print(f"cnn_step029 GA5 C={GA5_CH} T1 | {args.epochs}ep 50%data | Mixup α={args.mixup_alpha}")
    print(f"params={n_p:,}  MACs={macs:.1f}M  (GA2: 481K/98.5M | Ref: 583K/215.3M)")
    print(f"Baseline={BASELINE:.4f}  Advance>={ADVANCE:.4f}")
    ...
    print(f"\n{'='*60}\ncnn_step029 RESULT — GA5 C={GA5_CH} T1\n{'='*60}")
    print(f"  best={best:.4f} @ep{best_ep}  Δ_GA2={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    adv = "ADVANCE → T2 multi-seed GA5 (step030)" if verdict == "ADVANCE" else "NO-GAIN → try GA5b wider or deeper"
    print(f"  -> {adv}")
