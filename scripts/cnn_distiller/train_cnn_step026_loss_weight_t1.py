"""CNN distillation step026 — loss weight ablation, GA2 + Mixup(α=0.2).
Question: Does reducing feat_cos weight (0.50→0.20-0.30) and boosting DKD
          give more teacher-consistent learning → close 0.29pp gap to STRONG?
Context: step022 GA2+Mixup T2: 77.32%±0.56% EFF-PARETO (gap −0.29pp to STRONG).
         step025 LS T1: CLOSED — over-regularization atop Mixup.
         Current loss: 0.50·feat + 0.30·dkd + 0.20·ce.
Hypothesis: DKD more informative than feat_cos. Shifting weight feat→DKD
            gives more teacher-guided learning → lower seed variance at T2.
Sweep: (feat_w, dkd_w, ce_w):
  A = (0.30, 0.50, 0.20)  — reduce feat, boost DKD
  B = (0.20, 0.60, 0.20)  — minimal feat, heavy DKD
Baseline: step021 GA2+Mixup T1 = 73.61%. Advance: ≥74.11% (+0.5pp).
Output: results/cnn_step026_loss_weight_t1__{SLOT}.json
"""
...
OUT = ROOT / "results" / f"cnn_step026_loss_weight_t1__{SLOT}.json"

GA2_CH, GA2_K, GA2_EXP, GA2_SIDE, GA2_CRELU = (32, 64, 384), 3, 2, False, True
STEP021_BEST = 0.7361
ADVANCE = STEP021_BEST + 0.005

# (feat_w, dkd_w, ce_w) configs to sweep
WEIGHT_CONFIGS = [
    ("A", 0.30, 0.50, 0.20),
    ("B", 0.20, 0.60, 0.20),
]
...
def distil_loss(logits, sf, tf, soft, lbl_a, lbl_b, lam, feat_w, dkd_w, ce_w):
    ...
    return feat_w * feat + dkd_w * kd + ce_w * ce
...
def run_config(name, feat_w, dkd_w, ce_w, sub, val_loader):
    ...
    print(f"\n{'─'*50}\nConfig {name}: feat={feat_w} dkd={dkd_w} ce={ce_w}")
    for ep in range(args.epochs):
        ...
        for img, tf_, soft, lbl in tr:
            ...
            logits, sf = model(img)
            loss = distil_loss(logits, sf, tf_, soft, lbl_a, lbl_b, lam, feat_w, dkd_w, ce_w)
    ...
    print(f"  -> {name}  best={best:.4f} @ep{best_ep}  Δ_step021={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    return {"best": best, "best_ep": best_ep, "delta": delta, "verdict": verdict,
            "elapsed": elapsed, "feat_w": feat_w, "dkd_w": dkd_w, "ce_w": ce_w}
...
def main():
    ...
    print(f"cnn_step026 GA2+Mixup(α={args.mixup_alpha}) Loss Weight Ablation T1 | {args.epochs}ep 50%data | seed={args.seed}")
    print(f"GA#2: C=(32,64,384) k=3 exp=2 crelu  params={n_p:,}  MACs={macs:.1f}M")
    print(f"Baseline (step021, feat=0.50 dkd=0.30 ce=0.20): {STEP021_BEST:.4f}  Advance>={ADVANCE:.4f}")
    results = {}
    for name, fw, dw, cw in WEIGHT_CONFIGS:
        results[name] = run_config(name, fw, dw, cw, sub, val_loader)
    best_name = max(results, key=lambda n: results[n]["best"])
    print(f"\n{'='*60}\ncnn_step026 SUMMARY — Loss Weight Ablation T1\n{'='*60}")
    print(f"  Baseline (feat=0.50 dkd=0.30 ce=0.20): {STEP021_BEST:.4f}")
    for name, fw, dw, cw in WEIGHT_CONFIGS:
        r = results[name]
        print(f"  {name} (feat={fw} dkd={dw} ce={cw}): {r['best']:.4f} @ep{r['best_ep']}  Δ={r['delta']:+.4f}  {r['verdict']}")
    winner = results[best_name]
    print(f"  Best: {best_name} → {winner['best']:.4f}")
    print(f"  -> {'ADVANCE to T2 multi-seed (step027)' if winner['verdict'] == 'ADVANCE' else 'NO-GAIN — close loss-weight direction'}")
    OUT.parent.mkdir(exist_ok=True)
    OUT.write_text(json.dumps({"configs": results, "best_config": best_name,
