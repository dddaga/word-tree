"""CNN distillation step036 — GA5 C=(48,96,384) k=3 side=True T0 scout.
Hypothesis: side branch (channel attention, Block1→Block2) untested in GA line.
All reference configs (Ref, D_small_s, F_wide, step004) use side=True — it's the default.
GA1-5 all use side=False (never ablated). SideBranch: ~17K params, ~1M MACs overhead.
Prior: GA5 k=3 T2=77.79% (step030) side=False. Baseline: GA5+Mixup T1=74.73% (step029).
...
OUT = ROOT / "results" / f"cnn_step036_ga5side_t0__{SLOT}.json"

GA5_CH, GA5_K, GA5_EXP, GA5_SIDE, GA5_CRELU = (48, 96, 384), 3, 2, True, True
GA5_NOSIDE_T1 = 0.7473   # step029 GA5 side=False T1 baseline
ADVANCE = 0.7523          # +0.50pp over no-side T1
...
def main():
    ...
    print(f"cnn_step036 GA5+side=True T0 | {args.epochs}ep 50%data | alpha={args.mixup_alpha}")
    print(f"  {DEVICE}  slot={SLOT}  params={n_p:,}  MACs={macs:.1f}M")
    print(f"  side=True (channel attention). Ref/D_small_s/F_wide all use side=True. GA5 side=False never ablated.")
    print(f"  Baseline={GA5_NOSIDE_T1:.4f} (GA5 side=False T1). Advance>={ADVANCE:.4f}")
    ...
    delta = best - GA5_NOSIDE_T1
    verdict = "ADVANCE" if best >= ADVANCE else "NO-GAIN"
    print(f"\n{'='*60}\ncnn_step036 RESULT — GA5+side=True T0\n{'='*60}")
    print(f"  best={best:.4f} @ep{best_ep}  Δ_GA5noside={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    adv = "ADVANCE → T1 GA5+side=True (step037)" if verdict == "ADVANCE" else "NO-GAIN → side branch no benefit; GA5 side=False optimal"
    print(f"  -> {adv}")
    OUT.parent.mkdir(exist_ok=True)
    OUT.write_text(json.dumps({"best": best, "best_ep": best_ep, "ga5_noside_t1": GA5_NOSIDE_T1,
                               "delta": delta, "verdict": verdict, "elapsed": elapsed,
                               "use_side_branch": GA5_SIDE, "channels": list(GA5_CH),
