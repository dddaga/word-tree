"""CNN distillation step032 — GA6 C=(64,128,384) T0 scout.
Hypothesis: C1/C2 width scales gains. GA5 (48,96) broke GA2 (32,64) ceiling by +0.47pp T2.
GA6 widens C1 32→64 (2×) and C2 64→128 (2×) vs GA2. Tests if width scaling continues.
Expect params ~600K, MACs ~180M (near Ref: 419K/183.2M). Still under F_wide (825K).
Advance: GA5+Mixup T1=74.73% +0.5pp = 75.23%. If advance → T1 multi-seed.
Config: GA6 C=(64,128,384) k=3 exp=2 crelu=True side=False mixup_alpha=0.2
Loss: 0.50·feat_cos + 0.30·dkd + 0.20·ce (Mixup wins over CutMix at T1 level)
"""
...
OUT = ROOT / "results" / f"cnn_step032_ga6_t0__{SLOT}.json"

GA6_CH, GA6_K, GA6_EXP, GA6_SIDE, GA6_CRELU = (64, 128, 384), 3, 2, False, True
GA2_T0 = 0.4132   # step007 (GA_2 T0 reproduction)
GA5_T1 = 0.7473   # step029 (GA5+Mixup T1, advance ref)
ADVANCE = 0.7523   # GA5_T1 + 0.50pp
...
def main():
    ...
    model = EfficientVGG(GA6_CH, GA6_K, GA6_EXP, GA6_SIDE, GA6_CRELU).to(DEVICE)
    n_p = sum(q.numel() for q in model.parameters())
    macs = count_macs(EfficientVGG(GA6_CH, GA6_K, GA6_EXP, GA6_SIDE, GA6_CRELU)) / 1e6
    opt = torch.optim.AdamW(model.parameters(), lr=3e-4, weight_decay=1e-4)
    sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=args.epochs, eta_min=1e-5)
    print(f"cnn_step032 GA6+Mixup T0 | {args.epochs}ep 50%data | alpha={args.mixup_alpha}")
    print(f"  {DEVICE}  slot={SLOT}  params={n_p:,}  MACs={macs:.1f}M")
    print(f"  GA6 C={GA6_CH} (GA2: 32/64/384, GA5: 48/96/384 → GA6: 64/128/384)")
    print(f"  Advance>={ADVANCE:.4f} (GA5+Mixup T1 +0.50pp)")
    ...
    print(f"\n{'='*60}\ncnn_step032 RESULT — GA6 C={GA6_CH} T0\n{'='*60}")
    print(f"  best={best:.4f} @ep{best_ep}  Δ_GA5mix={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
    adv = "ADVANCE → T1 GA6+Mixup (step033)" if verdict == "ADVANCE" else "NO-GAIN → GA6 width not beneficial"
    print(f"  -> {adv}")
    OUT.parent.mkdir(exist_ok=True)
    OUT.write_text(json.dumps({"best": best, "best_ep": best_ep, "ga5_t1_ref": GA5_T1,
                               "delta": delta, "verdict": verdict, "elapsed": elapsed,
                               "mixup_alpha": args.mixup_alpha, "channels": list(GA6_CH),
