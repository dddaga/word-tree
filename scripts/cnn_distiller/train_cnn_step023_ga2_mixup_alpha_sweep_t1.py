"""CNN distillation step023 — GA2 Mixup α sweep T1.
Question: Does higher Mixup α push mean past STRONG threshold?
Context: step022 GA2+Mixup(α=0.2) T2: 77.32% ±0.56% EFF-PARETO (gap −0.29pp to STRONG).
Seed=42 reached 78.06%, seed=1 dragged mean to 77.32%. More mixing may lift low seeds.
Baseline: step021 α=0.2 T1=73.61% @ep65. Advance: ≥74.11% (+0.5pp).
Configs: α ∈ {0.3, 0.4, 0.5} × seed=42. Best α → T2 multi-seed (cnn_step024).
Output: results/cnn_step023_ga2_mixup_alpha_sweep_t1__{SLOT}.json
"""
...
p.add_argument("--alphas", default="0.3,0.4,0.5")
...
OUT = ROOT / "results" / f"cnn_step023_ga2_mixup_alpha_sweep_t1__{SLOT}.json"

GA2_CH, GA2_K, GA2_EXP, GA2_SIDE, GA2_CRELU = (32, 64, 384), 3, 2, False, True
STEP021_BEST = 0.7361  # α=0.2 T1 baseline
ADVANCE = STEP021_BEST + 0.005  # +0.5pp
...
def run_alpha(alpha, sub, va):
    ...
    for ep in range(args.epochs):
        ...
        for img, tf_, soft, lbl in tr:
            ...
            img, tf_, soft, lbl_a, lbl_b, lam = mixup_batch(img, tf_, soft, lbl, alpha)
            logits, sf = model(img)
    ...
    print(f"  -> alpha={alpha}  best={best:.4f} @ep{best_ep}  Δ_step021={delta:+.4f}  {verdict}  ({elapsed:.0f}s)")
...
def main():
    ...
    print(f"cnn_step023 GA2 Mixup α-sweep T1 | {args.epochs}ep 50%data | seed={args.seed}")
    print(f"GA#2: C=(32,64,384) k=3 exp=2 crelu  params={n_p:,}  MACs={macs:.1f}M")
    print(f"Baseline (step021 α=0.2): {STEP021_BEST:.4f}  Advance>=+0.5pp: {ADVANCE:.4f}")
    print(f"Testing α ∈ {alphas}")
    results = {}
    for alpha in alphas:
        results[alpha] = run_alpha(alpha, sub, val_loader)
    best_alpha = max(results, key=lambda a: results[a]["best"])
    print(f"\n{'='*60}\ncnn_step023 SUMMARY — GA2 Mixup α-sweep T1\n{'='*60}")
    print(f"  step021 baseline (α=0.2): {STEP021_BEST:.4f}")
    for a in alphas:
        r = results[a]
        print(f"  α={a}: {r['best']:.4f} @ep{r['best_ep']}  Δ={r['delta']:+.4f}  {r['verdict']}")
    print(f"  Best α: {best_alpha} → {results[best_alpha]['best']:.4f}")
    winner = results[best_alpha]
    print(f"  -> {'ADVANCE to T2 multi-seed' if winner['verdict']=='ADVANCE' else 'NO-GAIN — investigate'}")
    OUT.parent.mkdir(exist_ok=True)
    OUT.write_text(json.dumps({"alphas": {str(a): results[a] for a in alphas},
                               "best_alpha": best_alpha, "baseline_step021": STEP021_BEST,
