"""CNN distillation step022 — GA2 + Mixup T2 multi-seed variance.
Launch if step021 (Mixup T1) ADVANCE >=73.01%.
Hypothesis: Mixup stabilizes ep25 peak, pushes mean above Ref=77.61%.
Config: GA#2 C=(32,64,384) k=3 exp=2 crelu=(T,T,F) mixup_alpha=0.2
seed=42 T2+Mixup best from step021 passed via --seed42_best or auto-loaded.
Output: results/cnn_step022_ga2_mixup_multiseed_t2__{SLOT}.json
"""
...
OUT = ROOT / "results" / f"cnn_step022_ga2_mixup_multiseed_t2__{SLOT}.json"
...
def main():
    ...
    print(f"cnn_step022 GA2+Mixup T2 multi-seed | {args.epochs}ep 100%data | alpha={args.mixup_alpha}")
    ...
    print(f"\n{'='*60}\ncnn_step022 SUMMARY — GA2+Mixup T2 (alpha={args.mixup_alpha})\n{'='*60}")
