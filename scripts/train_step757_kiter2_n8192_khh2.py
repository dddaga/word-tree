"""Step 757: K_iter=2 at N=8192 K_hh=2 T1 + inline inference latency benchmark.

K_hh=2 FIXED (vs step752 which uses K_hh=round(N/1000)=8).
PURPOSE: test whether K_hh=2 is universally good (efficiency config) or needs to
scale with N. Paired comparison with step752 (same N, K_iter; only K_hh differs).

Existing curve:
  N=8192 K_hh=8 K_iter=2: step752 (K_hh=round(N/1000) rule)
  N=8192 K_hh=2 K_iter=2: THIS STEP

If step757 ≥ step752: K_hh=2 universal.
If step757 < step752: K_hh must scale with N.
...
parser = argparse.ArgumentParser(
    description="Step 757: K_iter=2 @ N=8192 K_hh=2 T1 + inline latency bench"
)
...
OUT_PATH = ROOT / "results" / "train_step757_kiter2_n8192_khh2.json"
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 757 — K_iter=2 @ N=8192 K_hh=2 (fixed) T1 + inline latency")
    print(f"N={N} D={D} K_hh={K_HH} K_iter={K_ITER} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"K_hh=2 FIXED. Paired with step752 (K_hh=8). Threshold: ≥96%")
    print(f"{'='*70}")
    ...
    if best >= 0.96:
        print(f"  → K_hh=2 viable at N=8192 K_iter=2. Compare to step752 (K_hh=8) for K_hh decision.")
    else:
        print(f"  → Below threshold. Compare delta vs step752 to determine K_hh sensitivity.")
