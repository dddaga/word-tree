"""Step 171: warm+W_proj at N=1024 D=32 K_hh=4 — test K_hh lever on efficiency track.

MOTIVATION
==========
step169 (warm+W_proj D=32 full data 150ep): 94.01% — 0.99pp short of phase exit.
D=32 ceiling at N=1024 appears to be ~94.0% with K_hh=8.

K_hh=4 was confirmed winner at N=4096 (+0.56pp, −18% FLOPs, step86).
Hypothesis: sparser connectivity improves representational quality by reducing over-smoothing.
At N=1024, K_hh=4 means fewer neighbors but potentially better-separated routing.

This step tests whether K_hh=4 pushes warm+W_proj D=32 past 94.01%.
FLOPs at N=1024 D=32 K_iter=8 K_hh=4 ≈ 3.1M (half of step169, well within 6.18M budget).

Also tests alpha_ahebb=1.05 (confirmed +0.79pp at N=4096, step133-B).
And K_iter=12 with K_hh=4 (FLOPs ≈ 4.6M ≤ 6.18M ✓) — more iterations over sparser graph.

Teacher: K=12 D=32 K_hh=4 N=1024 (fresh — step168 teacher had K_hh=8, incompatible topology).

CONFIGS (N=1024, D=32, K_hh=4, K_in=25, AH=1.0/1.05, 50% data, 75ep — Tier-1)
=================================================================================
  Ref : K=8 scratch K_hh=4 D=32 (new baseline for K_hh=4)
  A   : warm+W_proj K_hh=4 K_iter=8  alpha=1.0  ← primary test
  B   : warm+W_proj K_hh=4 K_iter=8  alpha=1.05 ← alpha calibration (free param)
  C   : warm+W_proj K_hh=4 K_iter=12 alpha=1.0  ← more iterations (FLOPs ~4.6M)

Phase-exit criterion: ≥95% @ ≤6.18M FLOPs.

To reproduce:
    python -u scripts/train_step171_khh4_d32.py --device cpu
    python -u scripts/train_step171_khh4_d32.py --device mps
    python -u scripts/train_step171_khh4_d32.py --configs A --device mps
"""
...
OUT_PATH     = ROOT / "results" / "train_step171_khh4_d32.json"
...
CONFIGS = [
    Config("Ref", "Ref  K=8 scratch K_hh=4 D=32 (new K_hh=4 baseline)",          False, False, 8,  1.0),
    Config("A",   "A    warm+W_proj K_hh=4 K_iter=8  alpha=1.0  ← primary test",  True,  True,  8,  1.0),
    Config("B",   "B    warm+W_proj K_hh=4 K_iter=8  alpha=1.05 ← alpha tweak",   True,  True,  8,  1.05),
    Config("C",   "C    warm+W_proj K_hh=4 K_iter=12 alpha=1.0  ← more iters",    True,  True,  12, 1.0),
]
...
def get_teacher(force: bool = False) -> dict:
    if not force and TEACHER_PATH.exists():
        print(f"  Loading K_hh=4 D=32 teacher from {TEACHER_PATH}")
        return torch.load(TEACHER_PATH, map_location=DEVICE, weights_only=True)
    print(f"\nTraining N=1024 D=32 K_hh=4 teacher K={K_ITER_TEACHER} for {EPOCHS}ep...")
...
def make_model(cfg: Config, seed_offset: int = 0) -> nn.Module:
    r = _build(cfg.k_iter, SEED + seed_offset)
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 171 — warm+W_proj N=1024 D=32 K_hh=4 (efficiency track K_hh test)")
    print(f"N={N}  D={D}  K_hh={K_HH}  K_in={K_IN}  Epochs={EPOCHS}  Data=50%")
    print(f"Phase-exit: ≥95% @ FLOPs ~3.1M (K_iter=8) or ~4.6M (K_iter=12)")
    print(f"step169 reference (K_hh=8 full data): 94.01%\n{'='*70}")
    ...
    for i, cfg in active:
        ...
        if cfg.warm and ts:
            print(f"  Warm-loading from K_hh=4 teacher...")
            model.warm_load(ts)
        n_p = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"  params={n_p:,}  k_iter={cfg.k_iter}  alpha={cfg.alpha_ahebb}")
    ...
    print(f"\n{'='*70}\nSTEP 171 SUMMARY — warm+W_proj D=32 K_hh=4\n{'='*70}")
    ref = results.get("Ref", {}).get("top1_best", 0)
    print(f"step169 reference (K_hh=8 full data 150ep): 94.01%")
    print(f"step168-B reference (K_hh=8  50% data  75ep): 93.20%")
