"""Step 752: K_iter=2 at N=8192 K_hh=8 T1 + inline inference latency benchmark.

Phase 2 of latency-Pareto K_iter floor search.

Existing curve:
  N=8192 K_hh=8 K_iter=3: step751 (Phase 1)
  N=8192 K_hh=8 K_iter=2: THIS STEP
...
Next-step verdict:
  ≥96%: advance Phase 3 K_iter=1 @ N=8192 OR Phase 2 @ N=16384
  <96%: K_iter=2 floor reached; go to N=16384 K_hh=16 K_iter=2 (step754)
"""
...
parser = argparse.ArgumentParser(
    description="Step 752: K_iter=2 @ N=8192 K_hh=8 T1 + inline latency bench"
)
...
D = 16; K_HH = 8; K_IN = 25; K_ITER = 2
ALPHA_REFLECT = 0.5; ALPHA_TURING = 0.0
FLOPS = 3 * N * K_HH * D * K_ITER
OUT_PATH = ROOT / "results" / "train_step752_kiter2_n8192_khh8.json"
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 752 — K_iter=2 @ N=8192 K_hh=8 T1 + inline latency")
    ...
    if best >= 0.96:
        print(f"  → Phase 3: K_iter=1 @ N=8192 K_hh=8 (step753)")
        print(f"  → Also viable: K_iter=2 @ N=16384 K_hh=16 (step754)")
    else:
        print(f"  → K_iter=2 floor reached; next: N=16384 K_hh=16 K_iter=2 (step754)")
