"""Step 721: ΔW rotation vs projection at N=512 Tier-1.

step714 Tier-2 (projection): Ref=80.9%, A_proj=91.9%, Δ=+11pp @ 0.25M FLOPs.
step301 Tier-0 (rotation at N=2048): C_rot=94.39%(+2.52pp) > A_proj=94.01%(+2.11pp).
Rotation beats projection by ~0.4pp at N=2048.

QUESTION: Does rotation also beat projection at N=512 (high-gain regime)?
If A_rot >> A_proj at N=512, the efficiency frontier improves further.
If A_rot ≈ A_proj, the mechanisms are equivalent and projection simpler.

Configs:
  Ref    : AH α=1.0 (baseline)
  A_proj : ΔW projection (|Z_nb · ΔŴ|·Z_nb) — step714 result: +11pp
  A_rot  : ΔW rotation (rotate Z_nb in (Z_nb,ΔW) plane by learned θ)

Tier-1 (75ep, 50% data) for rapid comparison.
"""
...
OUT_PATH = ROOT / "results" / "train_step721_rotation_n512.json"


class SGNNET_DeltaAH(nn.Module):
    """ΔW projection or rotation, no AH.

    mode: 'proj' → Z_nb scaled by |Z_nb · ΔŴ|
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Step 721 — ΔW rotation vs projection at N=512 Tier-1")
    print(f"N={N} D={D} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"Prior: proj +11pp (step714 T2). Rot beat proj by +0.4pp at N=2048 (step301).")
    print(f"{'='*70}")
    ...
    if not math.isnan(d_rot) and not math.isnan(d_proj):
        ...
        print(f"  Winner: {winner} by {margin:.2f}pp at N=512")
