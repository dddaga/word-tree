"""Step 172: warm+W_proj at N=1024 D=48 K_hh=4 — representation capacity test.

MOTIVATION
==========
Efficiency track ceiling progression:
  D=16 (step167): 93.17% @ ~3.1M FLOPs
  D=32 (step169): 94.01% @ ~6.1M FLOPs  ← 0.99pp short of phase exit
  D=32 K_hh=4 (step171): running @ ~3.1M FLOPs

D=48 sits between D=32 (ceiling 94.01%) and D=64 (accuracy track 97.86%).
With K_hh=4 to keep FLOPs in budget:
  FLOPs at N=1024 D=48 K_hh=4 K_iter=8 ≈ 4.65M ≤ 6.18M ✓

Hypothesis: D=32 representation ceiling is a capacity bottleneck. D=48 gives
enough extra dimensions to push past 95% while staying within FLOPs budget.

Teacher: K=12 D=48 K_hh=4 N=1024 (fresh — topology must match K_hh=4).

CONFIGS (N=1024, D=48, K_hh=4, K_in=25, 50% data, 75ep — Tier-1)
==================================================================
  Ref : K=8 scratch D=48 K_hh=4 (new D=48 baseline)
...
D=48 FLOPs ~4.65M ≤ 6.18M ✓

To reproduce:
    python -u scripts/train_step172_d48_warmproj.py --device mps
    python -u scripts/train_step172_d48_warmproj.py --device cpu
    python -u scripts/train_step172_d48_warmproj.py --configs B --device mps
"""
...
D = 48; K_HH = 4; K_IN = 25; K_ITER_TEACHER = 12
ALPHA_REFLECT  = 0.5; ALPHA_TURING = 0.0

TEACHER_PATH = ROOT / "results" / "train_step172_teacher_d48.pt"
OUT_PATH     = ROOT / "results" / "train_step172_d48_warmproj.json"
...
CONFIGS = [
    Config("Ref", "Ref  K=8 scratch D=48 K_hh=4 (D=48 baseline)",          False, False, 1.0),
    Config("B",   "B    warm+W_proj D=48 K_hh=4 alpha=1.0  ← primary",     True,  True,  1.0),
    Config("C",   "C    warm+W_proj D=48 K_hh=4 alpha=1.05 ← alpha tweak", True,  True,  1.05),
]
...
def get_teacher(force: bool = False) -> dict:
    if not force and TEACHER_PATH.exists():
        print(f"  Loading D=48 K_hh=4 teacher from {TEACHER_PATH}")
        return torch.load(TEACHER_PATH, map_location=DEVICE, weights_only=True)
    print(f"\nTraining N=1024 D=48 K_hh=4 teacher K={K_ITER_TEACHER} for {EPOCHS}ep...")
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 172 — warm+W_proj N=1024 D=48 K_hh=4 (representation capacity test)")
    print(f"N={N}  D={D}  K_hh={K_HH}  K_in={K_IN}  Epochs={EPOCHS}  Data=50%")
    print(f"FLOPs ~4.65M ≤ 6.18M budget ✓")
    print(f"Phase-exit: ≥95% @ ≤6.18M FLOPs")
    print(f"step169 D=32 K_hh=8 reference (full data): 94.01%\n{'='*70}")
    ...
    for i, cfg in active:
        ...
        if cfg.warm and ts:
            print(f"  Warm-loading from D=48 K_hh=4 teacher...")
            model.warm_load(ts)
        n_p = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"  params={n_p:,}  D={D}  K_hh={K_HH}  alpha={cfg.alpha_ahebb}")
    ...
    print(f"\n{'='*70}\nSTEP 172 SUMMARY — warm+W_proj D=48 K_hh=4\n{'='*70}")
    ref = results.get("Ref", {}).get("top1_best", 0)
    print(f"step169 D=32 K_hh=8 full data reference: 94.01%")
