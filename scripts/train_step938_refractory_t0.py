"""Step 938: Refractory neurons T0 (20ep, 50% data).

Port of step16E (AH era, +4.31pp at D=16/N=512/β=0.7/α_r=2.0) to current ΔW-proj arch.
Suppresses neurons that fired strongly in the previous K_iter step: Z_t -= α_r·max(0, β|Z_{t-1}|)
Additive suppression — gate-death theorem does not apply.
Complements α_reflect (spatial diversity) with temporal diversity.

CONFIGS (N=2048, D=16, K_hh=2, K_iter=5, ΔW-proj + α_AH=1.0, 20ep/50%)
  Ref        standard ΔW-proj (no refractory)
  A_β07_αr2  β=0.7, α_r=2.0  (AH era winner)
  B_β05_αr1  β=0.5, α_r=1.0  (weaker suppression)
  C_β09_αr1  β=0.9, α_r=1.0  (strong threshold, mild suppression)

ADVANCE: any config ≥+0.5pp vs Ref → T1.
AH era caveat: step16E was D=16/N=512 — closer regime than step106 but still different.
"""
...
parser.add_argument("--configs", default="Ref,A_β07_αr2,B_β05_αr1,C_β09_αr1")
...
OUT_PATH = ROOT / "results" / f"train_step938_refractory_t0_seed{SEED}__{SLOT}.json"
STEP_REF = 0.9396
...
class SGNNET_Refractory(nn.Module):
    """Refractory suppression: Z_t -= α_r * max(0, β * |Z_{t-1}|)."""
...
CONFIG_SPEC = {
    "Ref":       ("ref",        None,  None),
    "A_β07_αr2": ("refractory", 0.7,   2.0),
    "B_β05_αr1": ("refractory", 0.5,   1.0),
    "C_β09_αr1": ("refractory", 0.9,   1.0),
}
...
def make_model(key):
    spec, beta, alpha_r = CONFIG_SPEC[key]
    r = make_base()
    if spec == "ref":
        return SGNNET_Ref(r)
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"step938 — Refractory neurons T0 (20ep, 50% data)")
    print(f"  device={DEVICE}  epochs={EPOCHS}  seed={SEED}")
    print(f"  N={N}/D={D}/K_hh={K_HH}/K_iter={K_ITER}")
    print(f"  AH era ref (step16E): +4.31pp at D=16/N=512/β=0.7/α_r=2.0")
    ...
    for key in keys:
        ...
        spec, beta, alpha_r = CONFIG_SPEC[key]
        print(f"{'─'*60}")
        print(f"{key}: spec={spec}  β={beta}  α_r={alpha_r}  params={n_p:,}")
        ...
        results[key] = {"spec": spec, "beta": beta, "alpha_r": alpha_r,
    ...
    print(f"\n{'='*70}")
    print(f"STEP 938 SUMMARY — Refractory neurons T0")
    print(f"{'='*70}")
