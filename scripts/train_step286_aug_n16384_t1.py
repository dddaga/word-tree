"""Step 286: Aug @ N=16384 T1 — extending the aug scaling curve.

MOTIVATION
==========
Aug N-scaling curve T2 (confirmed):
  N=1024:  +0.54pp  — step280
  N=2048:  +0.66pp  — step269
  N=4096:  +0.56pp  — step273
  N=8192:  +0.43pp  — step276

Does aug continue to help at N=16384? Does the gain continue to decrease?
N=16384 no-aug would be the new upper bound on the scaling curve.
...
OUT_PATH = ROOT / "results" / f"train_step286_aug_n16384_t1_seed{SEED}__{SLOT}.json"

CONFIGS = {
    "Ref_n16384":   (False, "N=16384 no-aug T1 ref"),
    "A_n16384_aug": (True,  "N=16384 + aug T1 (scaling curve extension)"),
}
...
def main():
    ...
    print(f"Step 286 — Aug @ N=16384 T1 ({EPOCHS}ep, 50% data)")
    ...
    print(f"\n{'='*60}\nSTEP 286 SUMMARY — Aug @ N=16384 T1")
