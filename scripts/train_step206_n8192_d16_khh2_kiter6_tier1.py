"""Step 206: N=8192 D=16 K_hh=2 K_iter=6 Tier-1 — N-scaling law next doubling.

N-scaling at D=16 K_hh=2 K_iter=6:
  N=1024 (step198 T1): 88.92% @ 0.59M — killed
  N=2048 (step195 T2): 96.08% @ 1.18M ✓ ≤1% FLOPs criterion MET
  N=4096 (step201 T1): 95.64% @ 2.36M ✓ T1 phase exit
  N=4096 (step204 T2): ~96.9%+ @ 2.36M ✓ running
  N=8192 (this T1):    ?      @ 4.72M FLOPs ← next doubling

FLOPs = 3×8192×2×16×6 = 4,718,592 ≈ 4.72M — within ≤6.18M budget.

N-scaling pattern: each N-doubling adds ~+1-2pp. N=4096 T1=95.64%, so N=8192 T1
projected ~97%+. This would validate strong N-scaling law at D=16.
"""
...
OUT_PATH = ROOT / "results" / "train_step206_n8192_d16_khh2_kiter6_tier1.json"


def main():
    print(f"\n{'='*70}")
    print(f"Step 206 — N=8192 D=16 K_hh=2 K_iter=6 Tier-1 (50% data 75ep)")
    print(f"FLOPs={FLOPS:,} (~{FLOPS/1e6:.2f}M)")
    print(f"N-scaling: N=2048→96.08%, N=4096→95.64% T1 (→96.9%+ T2)")
    print(f"N=8192 projected: ~97%+ T1 if scaling holds")
    print(f"{'='*70}")
    ...
    result = {"A": {"N": N, "D": D, "K_hh": K_HH, "K_iter": K_ITER,
                    ...
                    "label": "A scratch N=8192 D=16 K_hh=2 K_iter=6 α=1.0 50% 75ep"}}
    OUT_PATH.parent.mkdir(exist_ok=True)
    OUT_PATH.write_text(json.dumps(result, indent=2))
    n4096_t1 = 0.9564
    print(f"\n{'='*70}")
    print(f"STEP 206: {best:.4f}  vs_N4096_T1={best-n4096_t1:+.4f}  FLOPs={FLOPS/1e6:.2f}M")
