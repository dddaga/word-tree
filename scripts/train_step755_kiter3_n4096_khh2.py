"""Step 755: K_iter=3 at N=4096 K_hh=2 T1 + inline inference latency benchmark.

K_hh=2 FIXED (vs step750 which uses K_hh=round(N/1000)=4).
PURPOSE: test whether K_hh=2 is universally good (efficiency config) or needs to
scale with N. Paired comparison with step750 (same N, K_iter; only K_hh differs).

Existing curve:
  N=4096 K_hh=4 K_iter=3: step750 (K_hh=round(N/1000) rule)
  N=4096 K_hh=2 K_iter=3: THIS STEP

If step755 ≥ step750: K_hh=2 universal → apply to all N in latency-Pareto track.
If step755 < step750: K_hh must scale with N → keep per-N rule.
...
OUT_PATH = ROOT / "results" / "train_step755_kiter3_n4096_khh2.json"
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 755 — K_iter=3 @ N=4096 K_hh=2 (fixed) T1 + inline latency")
    print(f"N={N} D={D} K_hh={K_HH} K_iter={K_ITER} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"K_hh=2 FIXED (efficiency config). Paired with step750 (K_hh=4).")
    print(f"Threshold: ≥96%")
    print(f"{'='*70}")
    ...
    if best >= 0.96:
        print(f"  → K_hh=2 viable at N=4096 K_iter=3. Compare to step750 (K_hh=4) for K_hh decision.")
