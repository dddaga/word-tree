"""Step 278: Aug @ N=1024 T1 — bottom of the scaling curve.

MOTIVATION
==========
Aug scaling curve (T2 confirmed, T1 trend):
  N=2048+aug T2: 96.13% (+0.66pp) — step269
  N=4096+aug T2: 97.68% (+0.56pp) — step273
  N=8192+aug T2: running — step276

N=1024 is missing from the curve. Adding it completes the full N-scaling plot
for the paper (N=1024/2048/4096/8192 with and without aug).

At N=1024 D=16, we expect lower accuracy (~88-92%) but the aug delta should
still be present. If Δ is consistent, aug is truly scale-invariant.

CONFIGS
=======
  Ref_n1024   : N=1024, no-aug
  A_n1024_aug : N=1024, aug

Tier: T1 (75ep, 50% data)

To run:
    python -u scripts/train_step278_aug_n1024_t1.py --device mps
    python -u scripts/train_step278_aug_n1024_t1.py --device cuda
"""
...
OUT_PATH = ROOT / "results" / f"train_step278_aug_n1024_t1_seed{SEED}__{SLOT}.json"

CONFIGS = {
    "Ref_n1024":   (False, "N=1024 no-aug baseline"),
    "A_n1024_aug": (True,  "N=1024 + aug (bottom of scaling curve)"),
}
...
def main():
    ...
    print(f"Step 278 — Aug @ N=1024 T1 ({EPOCHS}ep, 50% data)")
    ...
    for key in keys:
        ...
        print(f"{'─'*60}")
        print(f"{key}: {desc}  params={n_p:,}  routing={routing_macs/1e3:.0f}K MACs")
    ...
    print(f"\n{'='*60}")
    print(f"STEP 278 SUMMARY — Aug @ N=1024 T1 (scaling curve bottom)")
