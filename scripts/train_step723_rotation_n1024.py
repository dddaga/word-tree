"""Step 723: ΔW rotation vs projection at N=1024 Tier-1.

Finding: proj beats rot at N=512 by 5.94pp (step721).
         rot beats proj at N=2048 by ~0.4pp (step301).
         → Crossover is somewhere between N=512 and N=2048.

This step tests N=1024 to locate the crossover:
  If proj > rot at N=1024: crossover is between N=1024 and N=2048
  If rot > proj at N=1024: crossover is between N=512 and N=1024

Paper implication: proj = preferred mechanism for high-headroom (small N) regime;
rot = marginal advantage only near the D=16 ceiling.

Configs:
  Ref    : AH α=1.0
  A_proj : ΔW projection (+4.87pp known T1, +4.68pp T2 — step707)
  A_rot  : ΔW rotation (step301: +2.52pp T0 at N=2048)
...
OUT_PATH = ROOT / "results" / "train_step723_rotation_n1024.json"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Step 723 — ΔW rotation vs projection at N=1024 Tier-1 (crossover search)")
    print(f"N={N} D={D} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"step721 N=512: proj=+12.59pp >> rot=+6.65pp (proj wins +5.94pp)")
    print(f"step301 N=2048: rot=+2.52pp > proj=+2.11pp (rot wins +0.41pp)")
    print(f"QUESTION: does proj still beat rot at N=1024?")
    print(f"{'='*70}")
    ...
    if not math.isnan(d_rot) and not math.isnan(d_proj):
        ...
        print(f"  Winner: {winner} by {margin:.2f}pp at N=1024")
        if winner == "A_proj":
            print(f"  → Crossover is between N=1024 and N=2048")
        else:
            print(f"  → Crossover is between N=512 and N=1024")
