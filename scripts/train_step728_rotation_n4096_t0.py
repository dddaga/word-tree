"""Step 728: ΔW rotation vs projection at N=4096 Tier-0 scout.

Does rot > proj extend beyond N=2048 (the crossover point) to the D=16 ceiling?
step704 showed A_proj HURTS at N=4096 (−0.74pp). Does rot also hurt, or does rot
preserve its advantage near the ceiling?

Prediction if rot wins at ceiling:
  → Rotation is specifically suited to near-ceiling routing (small absolute gain regime)
  → The crossover is a feature: proj=high-headroom mechanism, rot=low-headroom mechanism

Prediction if both hurt at N=4096:
  → Both mechanisms are N-specific; neither generalises to very large N

Tier-0 scout (20ep, 50% data) — fast rejection filter.
"""
...
OUT_PATH = ROOT / "results" / "train_step728_rotation_n4096_t0.json"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Step 728 — ΔW rot vs proj N=4096 Tier-0 scout (ceiling behaviour)")
    print(f"N={N} D={D} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"Known: proj HURTS at N=4096 (−0.74pp, step704). Does rot help or also hurt?")
    print(f"{'='*70}")
    ...
    if not math.isnan(d_rot) and not math.isnan(d_proj):
        if d_rot > 0 and d_proj < 0:
            print(f"  → ROT SURVIVES CEILING: rot helps (+{d_rot*100:.2f}pp), proj still hurts ({d_proj*100:+.2f}pp)")
        elif d_rot > 0 and d_proj > 0:
            print(f"  → BOTH HELP at N=4096 — unexpected, needs T1 verification")
        elif d_rot < 0 and d_proj < 0:
            print(f"  → BOTH HURT at N=4096 — mechanism is N-specific, doesn't survive ceiling")
        else:
            print(f"  → proj helps, rot hurts — projection survives better at ceiling (unexpected)")
