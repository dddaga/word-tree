"""Step 284: K_in=15 + aug compound @ N=1024 T2.

MOTIVATION
==========
step283 T1: C_k15_aug_n1024 = 89.78% (+1.66pp vs 88.13% ref T1). Strong advance.
T2 validates compound at N=1024 (bottom of scaling curve):
  N=1024+aug T2 = 91.11% (step280) — aug-only baseline
  If K_in=15 costs ~−0.33pp (consistent), compound ~90.78%
  But aug dominates at small N, so compound may be net positive still.

Complete K_in=15+aug T2 curve will be:
  N=1024 T2: this run
  N=2048 T2: 95.46% (+0.18pp) — step274 DONE
  N=4096 T2: 97.30% (+0.18pp) — step279 DONE
  N=8192 T2: running — step282
...
parser.add_argument("--configs",  default="Ref_n1024,C_k15_aug_n1024")
...
OUT_PATH = ROOT / "results" / f"train_step284_kin15_aug_n1024_t2_seed{SEED}__{SLOT}.json"

CONFIGS = {
    "Ref_n1024":       (25, False, "N=1024 K_in=25 no-aug T2 ref"),
    "C_k15_aug_n1024": (15, True,  "N=1024 K_in=15 + aug T2 (COMPOUND bottom of curve)"),
}
...
def main():
    ...
    print(f"Step 284 — K_in=15+aug compound @ N=1024 T2 ({EPOCHS}ep, 100% data)")
    ...
    print(f"\n{'='*60}")
    print(f"STEP 284 SUMMARY — K_in=15+aug compound @ N=1024 T2")
    ...
    for k in keys:
        ...
        print(f"  {k:<26} K_in={r['K_in']} aug={'y' if r['use_aug'] else 'n'}  "
