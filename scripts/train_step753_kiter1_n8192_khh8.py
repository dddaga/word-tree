"""Step 753: K_iter=1 at N=8192 K_hh=8 T1 + inline inference latency benchmark.

Phase 3 of latency-Pareto K_iter floor search. Aggressive — likely below 96%
but measures the absolute floor and provides the latency number.

Existing curve:
  N=8192 K_hh=8 K_iter=3: step751
  N=8192 K_hh=8 K_iter=2: step752
  N=8192 K_hh=8 K_iter=1: THIS STEP (floor probe)
...
Threshold: ≥96% (likely to fail — this step measures the floor).
"""
...
parser = argparse.ArgumentParser(
    description="Step 753: K_iter=1 @ N=8192 K_hh=8 T1 + inline latency bench (floor probe)"
)
...
D = 16; K_HH = 8; K_IN = 25; K_ITER = 1
ALPHA_REFLECT = 0.5; ALPHA_TURING = 0.0
FLOPS = 3 * N * K_HH * D * K_ITER
OUT_PATH = ROOT / "results" / "train_step753_kiter1_n8192_khh8.json"
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 753 — K_iter=1 @ N=8192 K_hh=8 T1 + inline latency (floor probe)")
    print(f"N={N} D={D} K_hh={K_HH} K_iter={K_ITER} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"Threshold: ≥96% (aggressive — this step measures the K_iter floor)")
    print(f"{'='*70}")
    ...
    results = {"best": {"top1_best": best, "best_epoch": bep, "top1_history": top1h,
               ...
               "latency": bench}
    verdict = "≥96% VIABLE" if best >= 0.96 else ("borderline 94-96%" if best >= 0.94 else "<94% floor")
    print(f"\n{'='*70}")
    print(f"VERDICT: {verdict}  ({best:.4f})")
    if best >= 0.96:
        print(f"  → K_iter=1 viable: latency floor reached with accuracy maintained")
    else:
        print(f"  → K_iter floor confirmed at K_iter=2 (if step752 passed) or K_iter=3")
        print(f"  → Latency number still useful for Pareto plot even if accuracy fails")
