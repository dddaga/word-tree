"""Step 986: CIFAR-10 N=16384 T1 (75ep, 50% data).
Resolves step984 T0 non-monotonic result (best=80.23% @ep17 — underfit).
T1 tests whether N=16384 recovers scaling trend with more epochs.
N-scaling context: N2048=80.57%, N4096=82.53%, N8192=83.55%.
Advance: N=16384 >= 83.55% (N=8192 T2) → scaling holds → extend curve.
Usage:
    python scripts/train_step986_cifar10_n16384_t1.py --device cpu
"""
...
OUT_PATH = ROOT / "results" / f"train_step986_cifar10_n16384_t1_seed{SEED}__{SLOT}.json"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"step986 — CIFAR-10 N=16384 T1 (75ep, 50% data)")
    print(f"  device={DEVICE}  epochs={EPOCHS}  seed={SEED}  batch={BATCH}")
    print(f"  N={N} D={D} K_in={K_IN} K_hh={K_HH} K_iter={K_ITER}")
    print(f"  step984 T0: best=80.23% @ep17 (underfit at 20ep)")
    print(f"  N-scaling: N2048=80.57  N4096=82.53  N8192=83.55")
    print(f"  Advance: >= 83.55pp → scaling holds")
    print(f"{'='*70}\n")
    ...
    N8192 = 0.8355  # 83.55% as fraction
    print(f"\n{'='*70}")
    print(f"RESULT: best={best:.4f} @ep{best_ep}  elapsed={elapsed:.0f}s")
    print(f"Scaling: 2K=80.57  4K=82.53  8K=83.55  16K(T1)={best*100:.2f}")
    if best >= N8192:
        print(f"  → SCALING HOLDS: N=16384 >= N=8192 ({best*100:.2f}% >= 83.55%) → paper curve extended")
    else:
        print(f"  → NON-MONOTONIC CONFIRMED: N=16384={best*100:.2f}% < N=8192=83.55% → ceiling hit")
    print(f"→ {OUT_PATH}")
