"""Step 282: K_in=15 + aug compound @ N=8192 T2.

MOTIVATION
==========
step281 T1: C_k15_aug_n8192 = 96.05% (+1.15pp vs 94.90% ref T1). Strong advance.
T2 validates compound at N=8192 (top of scaling curve):
  N=8192+aug T2 = 97.38% (step276) — aug-only baseline
  If K_in=15 costs −0.33pp (consistent N=2048/4096), compound ~97.05%

Paper claim: K_in=15+aug delivers >97% at N=8192 with 26.7× seed reduction.
...
OUT_PATH = ROOT / "results" / f"train_step282_kin15_aug_n8192_t2_seed{SEED}__{SLOT}.json"

CONFIGS = {
    "Ref_n8192":       (25, False, "N=8192 K_in=25 no-aug (step209 ref ~97.17%)"),
    "C_k15_aug_n8192": (15, True,  "N=8192 K_in=15 + aug (COMPOUND T2)"),
}
...
def main():
    ...
    print(f"Step 282 — K_in=15+aug compound @ N=8192 T2 ({EPOCHS}ep, 100% data)")
    ...
    print(f"\n{'='*60}")
    print(f"STEP 282 SUMMARY — K_in=15+aug compound @ N=8192 T2")
