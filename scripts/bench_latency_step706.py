"""bench_latency_step706: Standalone inference-latency benchmarker for step706 config.

Back-fills the latency number for step706 (N=2048 K_hh=2 K_iter=5 D=16, ΔW proj)
for the Pareto plot.

Forward pass uses SGNNET_DeltaAH with mode="proj" — identical to step760's
implementation (copied here to keep bench scripts self-contained).

Protocol:
  - Build fresh step706 config (ΔW projection)
...
Device: 5060ti_cuda preferred (to compare against VGG16 FC ~0.07ms ref and
step199 AH-only baseline from bench_latency_step199.py).
"""
...
parser = argparse.ArgumentParser(
    description="bench_latency_step706: inference latency for step706 config (ΔW proj, N=2048)"
)
...
STEP_NAME = "step706_dw_proj"
OUT_PATH = ROOT / "results" / "bench_latency_step706.json"


# ── ΔW projection forward pass (from step760 SGNNET_DeltaAH, mode="proj") ────

class SGNNET_DeltaAH(nn.Module):
    """ΔW projection mechanism. Identical to step760 implementation."""
...
def main():
    print(f"\n{'='*70}")
    print(f"bench_latency_step706 — ΔW proj N=2048 K_hh=2 K_iter=5 D=16")
    ...
    print(f"  Compare to step199 AH-only: run bench_latency_step199.py")
    ...
    results = {"config": {"step": STEP_NAME, "N": N, "D": D, "K_hh": K_HH, "K_iter": K_ITER,
                          "flops": FLOPS, "n_params": n_p, "mechanism": "DeltaW-proj",
                          "device": str(DEVICE)},
               "latency": bench,
