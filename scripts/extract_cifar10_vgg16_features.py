"""Extract VGG16 conv features from CIFAR-10 for cross-dataset SGNNET training.
...
VGG16 features of CIFAR-10 (not ImageNet-trained Imagenette data), report accuracy.

OUTPUT FORMAT (matches src/training/dataset.py H5Dataset):
  data/store_cifar10.h5:
    /train/features     [50000, 25088] float32
    /train/soft_labels  [50000, 10]    float32 (one-hot)
    /train/labels       [50000]        int64
    /val/features       [10000, 25088] float32
    /val/soft_labels    [10000, 10]    float32
    /val/labels         [10000]        int64

Requires torchvision. Runs on MPS preferably; CPU acceptable (30-60 min).

Usage:
    d_env/bin/python3 scripts/extract_cifar10_vgg16_features.py --device mps
"""
...
parser.add_argument("--output", default="data/store_cifar10.h5")
...
def main():
    ...
    train_ds = torchvision.datasets.CIFAR10(
        root=str(ROOT / args.data_dir), train=True, download=False, transform=transform)
    val_ds = torchvision.datasets.CIFAR10(
        root=str(ROOT / args.data_dir), train=False, download=False, transform=transform)
