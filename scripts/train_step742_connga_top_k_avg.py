"""Step 742: ConnGA v2 with top_k_avg scoring (k=2).

Reuses step740's algorithm entirely. Only change: --scoring top_k_avg --top_k 2.
Equal weight on the top-2 children only; all others get zero weight.

Motivation: top_k_avg with k=2 enforces hard elitism — only the two best
children contribute to the gene pool. This maximises selection pressure
compared to rank (step741) and softmax (step740).
...
Config (identical to step740 except scoring):
  ...
  scoring=top_k_avg, top_k=2  (hardcoded defaults)

Compare to:
  step740: scoring=softmax (baseline)
  step741: scoring=rank
"""
...
parser = argparse.ArgumentParser(
    description="Step 742: ConnGA v2 with top_k_avg scoring, k=2 (hardcoded defaults)"
)
parser.add_argument("--device",   default="auto")
parser.add_argument("--epochs",   type=int,   default=30)
parser.add_argument("--gens",     type=int,   default=4)
parser.add_argument("--pop",      type=int,   default=8)
parser.add_argument("--scoring",  default="top_k_avg",
                    choices=["softmax", "rank", "top_k_avg"],
                    help="Scoring scheme — default hardcoded to 'top_k_avg'")
parser.add_argument("--tau",      type=float, default=0.5)
parser.add_argument("--top_k",    type=int,   default=2,
                    help="k for top_k_avg (default: 2)")
...
N_GENS   = args.gens
POP_SIZE = args.pop
SCORING  = args.scoring   # "top_k_avg" by default
TAU      = args.tau
TOP_K    = args.top_k     # 2 by default
...
def run_connga(tr, va):
    print(f"\n{'='*70}")
    print(f"ConnGA v2 (top_k_avg k={TOP_K}) | N={N} D={D} K_hh={K_HH} K_iter={K_ITER}")
    print(f"  {N_GENS} gens × {POP_SIZE} children × {EPOCHS} ep  |  scoring={SCORING} k={TOP_K}")
    print(f"{'='*70}")
    ...
    return {"algorithm": f"ConnGA_v2_top_k_avg_k{TOP_K}", "N": N, "D": D, "K_hh": K_HH,
            "K_iter": K_ITER, "flops": FLOPS, "n_gens": N_GENS, "pop_size": POP_SIZE,
            "epochs_per_child": EPOCHS, "scoring": SCORING, "top_k": TOP_K, "mut_prob": MUT_PROB,
            "gen_records": gen_records, "best_seen_acc": round(best_global_acc, 4),
            "final_acc": round(final_acc, 4), "final_history": final_hist,
...
def main():
    print(f"\n{'='*70}")
    print(f"{STEP_NAME}")
    print(f"ConnGA v2 — top_k_avg k={TOP_K} scoring (step742)")
    print(f"N={N} D={D} K_hh={K_HH} K_iter={K_ITER}  Device={DEVICE}")
    print(f"Scoring: {SCORING} k={TOP_K}  (step740 used softmax; step741 used rank)")
    print(f"{'='*70}")
    ...
    results = {"Ref": ref_result, f"ConnGA_v2_top_k_avg_k{TOP_K}": ga_result,
               "meta": {"step": STEP_NAME, "device": str(DEVICE), "seed": SEED,
                        ...
                        "scoring": SCORING, "top_k": TOP_K, "mut_prob": MUT_PROB,
                        "total_elapsed_s": round(time.time() - t0, 1)}}
    OUT_PATH.parent.mkdir(exist_ok=True)
    OUT_PATH.write_text(json.dumps(results, indent=2))

    ref_best = ref_result["best_acc"]; ga_best = ga_result["final_acc"]; delta = ga_best - ref_best
    verdict = ("TOP_K_AVG BETTER" if delta > 0.010 else
               "NEUTRAL" if delta > -0.005 else "TOP_K_AVG HURTS")
    print(f"\n{'='*70}")
    print(f"{STEP_NAME} SUMMARY")
    print(f"  Ref (static):                 {ref_best:.4f}")
    print(f"  ConnGA v2 top_k_avg final:    {ga_best:.4f}  Δ={delta:+.4f}")
