"""Step 263: K_iter=4 ΔW proj at N=4096 — does K=4 advantage scale?

MOTIVATION
==========
step262 3-seed T2 at N=2048: K=4 mean +0.02pp vs K=5 — statistically equivalent, 20% cheaper.
Open question: does the K=4 equivalence hold at N=4096 (D=16 ceiling scale)?

Prior N=4096 data (all at K_hh=2 K_iter=5):
  step203 T1: 96.08%  step205 T2: 97.17% (D=16 ceiling)

If K=4 ΔW proj at N=4096 matches K=5 within noise → paper Pareto story extends to ceiling.

CONFIGS (N=4096 D=16 K_hh=2 ΔW proj, seed=42, 75ep 50% T1)
  Ref : K_iter=5 (ΔW proj)
  A   : K_iter=4 (ΔW proj)
"""
...
OUT_PATH = ROOT / "results" / f"train_step263_kiter4_n4096_seed{SEED}.json"

CONFIGS = [
    dict(key="Ref", K_iter=5, label="ΔW proj K_iter=5 at N=4096"),
    dict(key="A",   K_iter=4, label="ΔW proj K_iter=4 at N=4096"),
]
...
def main():
    ...
    print(f"Step 263 — K_iter=4 ΔW proj at N={N} (Tier-1, 50% data)")
    print(f"  N={N} D={D} K_hh={K_HH} {EPOCHS}ep seed={SEED}")
    print(f"  Target: K=4 within ±0.2pp of K=5 (mirrors N=2048 finding)")
    ...
    print(f"\n\n========== STEP 263 SUMMARY (N={N} seed={SEED}) ==========")
    for cfg in active:
    ...
    if "Ref" in results and "A" in results:
        ...
        print(f"\nΔ(K_iter=4 − K_iter=5): {d:+.2f}pp at N={N}")
