"""Step 718: ΔW projection N=64 Tier-2 (150ep, 100% data, paper-ready).

step717 Tier-1 results:
  mini:cpu:  Ref=37.22%  A_proj=52.82%  Δ=+15.60pp
  studio:mps: Ref=36.54%  A_proj=53.07%  Δ=+16.53pp  (avg Δ≈+16pp)

SURPRISING: curve is non-monotone — N=128 peaks at +24pp, N=64 drops to +16pp.
Tier-2 needed to confirm the peak and fit the power law properly.

Headroom curve Tier-2 so far:
  N=2048: +1.56pp  N=1024: +4.68pp  N=512: +10.96pp  N=256: +20.02pp
  N=128:  +23.9pp  N=64: ?? (expected ~+16pp from Tier-1)

FLOPs=30,720 (0.03M) — 3% of original 0.98M record.
"""
...
OUT_PATH = ROOT / "results" / "train_step718_delta_n64_tier2.json"
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"Step 718 — ΔW proj N=64 Tier-2 (150ep 100% data, paper-ready)")
    print(f"N={N} FLOPs={FLOPS/1e6:.3f}M (3% of old 0.98M record)  Device={DEVICE}")
    print(f"Tier-1: Ref≈37%  A_proj≈53%  Δ≈+16pp")
    print(f"GOAL: confirm non-monotone peak. N=128(+23.9pp) > N=64(?) < N=256(+20pp)?")
    print(f"{'='*70}")
    ...
    if not math.isnan(delta):
        print(f"Ref={ref:.4f}  A_proj={proj:.4f}  Δ={delta:+.4f} ({delta*100:+.2f}pp)")
        print(f"Curve T2: N=64({delta*100:+.1f}) N=128(+23.9) N=256(+20.0) N=512(+11.0) N=1024(+4.7) N=2048(+1.6)")
        if delta * 100 < 20.0:
            print(f"  → NON-MONOTONE CONFIRMED: N=128 is local maximum in ΔW headroom curve")
        else:
            print(f"  → MONOTONE: curve continues rising at N=64")
