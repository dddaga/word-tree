"""Step 751: K_iter=3 at N=8192 K_hh=8 T1 + inline inference latency benchmark.

K_hh = round(N/1000) = 8. Threshold raised to 96% (step750 at 95.06% no longer qualifies).

Existing curve:
  N=4096 K_hh=4 K_iter=3: 95.06% (step750) — below new 96% threshold
  N=8192 K_hh=8 K_iter=3: THIS STEP
  N=8192 K_hh=2 K_iter=3: 94.93% T1 (step211 — diff K_hh)
...
D = 16; K_HH = 8; K_IN = 25; K_ITER = 3
ALPHA_REFLECT = 0.5; ALPHA_TURING = 0.0
FLOPS = 3 * N * K_HH * D * K_ITER
OUT_PATH = ROOT / "results" / "train_step751_kiter3_n8192_khh8.json"
...
def main():
    print(f"\n{'='*70}")
    print(f"Step 751 — K_iter=3 @ N=8192 K_hh=8 T1 + inline latency")
    print(f"N={N} D={D} K_hh={K_HH} K_iter={K_ITER} FLOPs={FLOPS/1e6:.3f}M  Device={DEVICE}")
    print(f"Threshold: ≥96% (raised from 95% after step750=95.06%)")
    print(f"{'='*70}")
    ...
    if best >= 0.96:
        print(f"  → advance Phase 2: K_iter=2 @ N=8192 K_hh=8 (step752)")
