"""Step 275: Aug @ N=8192 T1.

MOTIVATION
==========
Aug scaling curve so far:
  N=2048+aug T2: 96.13% (+0.66pp vs 95.47% ref) — step269
  N=4096+aug T2: 97.68% (+0.56pp vs 97.12% ref) — step273

Question: does aug continue to help at N=8192? N=8192 no-aug T2 = 97.17% (step209).
If aug gives ~+0.5pp, new ceiling ~97.7%, essentially matching N=4096+aug.
This would establish the aug gain as consistent across the N-scaling law.

CONFIGS
=======
  Ref_n8192   : N=8192, no-aug  (step209 ref ~97.17%)
  A_n8192_aug : N=8192, aug     (hypothesis: ~97.6-97.8%)

Tier: T1 (75ep, 50% data)

To run:
    python -u scripts/train_step275_aug_n8192_t1.py --device cuda
"""
...
OUT_PATH = ROOT / "results" / f"train_step275_aug_n8192_t1_seed{SEED}__{SLOT}.json"

CONFIGS = {
    "Ref_n8192":   (False, "N=8192 no-aug (step209 ref ~97.17%)"),
    "A_n8192_aug": (True,  "N=8192 + aug (ceiling check)"),
}
...
def main():
    ...
    print(f"Step 275 — Aug @ N=8192 T1 ({EPOCHS}ep, 50% data)")
    ...
    STEP209_REF = 0.9717
    print(f"\n{'='*60}")
    print(f"STEP 275 SUMMARY — Aug @ N=8192 T1")
