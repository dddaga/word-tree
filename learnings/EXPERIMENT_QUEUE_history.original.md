...
All DONE / KILLED / STALE experiment entries. Reference only.

---

## P-PAPER — Paper-blocking ablations and synthesis experiments (2026-04-14) — ALL DONE

| Step | Description | Scale | FLOPs | Script | Status |
|------|-------------|-------|-------|--------|--------|
| **step300** | **Polarizer α Tier-1 sweep** — α={0.5,1.0,1.5,2.0,2.5}. **DONE: A200 (α=2.0) = 94.73% winner (+0.84pp)**. step217b's α=1.5 no longer reproduces at efficiency config. CONFIRMED α=2.0-2.5 new optimum. | N=2048 D=16 | ~2M | ✅ | **DONE** |
...
| **step306** | **Activation retention mechanism** — **DONE: ALL variants HURT.** Ref=85.35% beats all (A_static_p10 -0.10pp, A_static_p30 -0.74pp, A_static_p50 -6.67pp, B_decay -3.24pp, C_norm_cons -18.67pp, D_reinject -4.41pp). Mechanism KILLED. | N=1024 D=16 | ~0.5M | ✅ | **DONE — KILLED** |
| **step320** | **F.normalize ablation (paper-critical)** — **DONE: F.normalize IS load-bearing.** Ref (l2)=91.62%, A (clamp only)=79.29% (−12.33pp), B (RMSNorm)=85.07% (−6.55pp). CONFIRMED paper claim. | N=2048 D=16 | 0.98M | ✅ | **DONE** |
| **step321** | **AH α sweep (paper-critical)** — **DONE: AH IS load-bearing.** α=0 collapses to 18.78%. α=0.25→0.75→1.0 monotonic (49→88→92%). α=1.0 optimal; α=1.25/1.50 slightly worse. CONFIRMED paper claim. | N=2048 D=16 | 0.98M | ✅ | **DONE** |
| **step400** | **CIFAR-10 generalization (paper-critical)** — **DONE (30ep mini_mps): Linear=37.94%, N512=33.38%, N2048=32.09%, N4096=33.32%. SGNNET UNDERPERFORMS LINEAR on raw pixels. Root cause: SGNNET designed for VGG16 feature inputs (25088-dim, high-level). Raw 3072-dim pixels are low-level — Fourier encoding on S^{D-1} not effective without feature extraction. Paper framing adjustment needed: SGNNET = classifier head requiring upstream features, not end-to-end.** | N=2048 D=16 | ~1M | ✅ | **PARTIAL — 75ep running studio_cpu** |
| **step401** | **Baselines (paper-critical)** — **DONE: Lin_direct=96.92%, MLP_64=97.20%, MLP_2=46.98%, MLP_3=45.91%, SGNNET_Ref=91.75% @20ep, SGNNET_RandProj=10.04% (chance).** CONFIRMED: W_pos is essential (+81pp vs random). MLP at same budget = 45-47%. Paper claim: SGNNET ≈ MLP_64 accuracy at 24× fewer params. | N=2048 | 0.98M | ✅ | **DONE** |
| **step402** | **N-scaling law curve** — **step402a DONE (mini_mps):** N=256: 59.39% @0.12M, N=512: 77.45% @0.25M, N=1024: 87.95% @0.49M, N=2048: 94.11% @0.98M. Log-linear scaling confirmed. step402b (N=4096,8192) still running on studio_cpu. | various | various | ✅ | **PARTIAL DONE** (N=256-2048 ✓, N=4096-8192 running studio_cpu) |
...
|------|-------------|--------|------------------|--------|--------|
| **step500** | **torch.compile + CUDA graphs bench** — **DONE (MASSIVE WIN):** V1 (torch.compile) = **4.2x training** / **6.5x inference** speedup vs V0 eager on 5060ti. GPU util 2.6% → **99.6%** (compute-bound). **Training 21,043 sps vs VGG 4,469 sps = 4.7x faster than VGG FC with 116x fewer FLOPs.** V2 neutral. V3 crashed (CUDA graph tensor overwrite bug — V1 supersedes). | CUDA | **4-6x** | ✅ | **DONE** |
| **step520** | **RCM index reordering** — **DONE: KILLED. V1=5.294ms, V_rcm=5.330ms → 0.993× speedup.** RCM is no-op at fp32 reduce-overhead: GPU util=98% (compute-bound, not BW-bound). RCM only helps when memory-BW-bound. | 4h | actual: ~1× | ✅ | **DONE — KILLED** |
| **step800** | **ncu FLOPs cross-check** — **DONE.** Analytical: old formula undercounts by 5.65×. True routing MACs = 2.13M. Total per-sample MACs = 1.85M. VGG FC = 247M → SGNNET = **0.75%** (under 1%). GPU util 0.09% at inference — memory-BW-bound. | 2h | validation | ✅ | **DONE** |
| **step801** | **max-autotune + bf16 benchmark** — **DONE.** V4 max-autotune = +3.5% (negligible). V5 bf16+scaler = 4.4× SLOWER (GradScaler overhead). V6 bf16+autotune+scaler = 4.4× SLOWER. **Decision: drop max-autotune (marginal gain). bf16 training with scaler = DO NOT USE.** | 1h | actual: +3.5% | ✅ | **DONE** |
| **step802** | **bf16 without GradScaler** — **DONE.** V7 bf16 no-scaler train = 5.207ms (+1.8% vs V1 5.299ms). Inference: +3–15% speedup at all batch sizes. **Decision: V7 = production inference config (free perf + half power draw). Training: V1≈V7, use either.** | 1h | actual: +1.8% train, +3-15% inf | ✅ | **DONE** |
| **step803** | **make_graphed_callables for fwd+bwd** — **DONE: KILLED. `cudaErrorStreamCaptureInvalidated` on fancy int64 indexing `Z[:, conn_hh, :]`. CUDA graph capture cannot handle dynamic int64 gather indices.** Fix requires Triton kernel (step530). | Medium (3-5h) | actual: 0× | ✅ | **DONE — KILLED** |

---

## P-SPEED — Completed (2026-04-14)

**Premise:** step610 confirmed K_iter ANNEALING/CURRICULUM fails at N=2048 efficiency config (all schedules −2 to −6pp). The network has fundamental dependence on K=5 sequential passes. New direction: keep the same total receptive field, but parallelize across fewer sequential steps.

**Prior findings (from audit):**
- Multi-hop precompute: **NOT TESTED** (novel)
- Parallel branches: step114 at N=1024 D=64 was "marginal" (demoted P3)
- Graph powers (A^k): wave-1 experiments KILLED (but that was amplitude-phase coupling, different mechanism)
- Wider+shallower: step213 (K_hh=3 at N=8192) no ceiling break; step214/215 (K_hh=8,16 at D=8) failed due to low D; but **K_hh=4,8 at N=2048 D=16 never tested**

| Step | Description | Metric targets | Script | Status |
|------|-------------|----------------|--------|--------|
| **step700** | **Multi-hop gather** — **DONE: ALL VARIANTS HURT.** Ref=92.28% @ 14.9ms. All multi-hop configs −3.5 to −13pp. Speedup ratios 0.73-1.04x (wider gather adds memory overhead). Best: A_k3_kh6=86.60% @ 16.5ms. Also SLOWER. **CONFIRMED: Multi-hop flattening loses per-iter nonlinearity stacking.** | 40-80% fewer passes at equal accuracy | ✅ | **DONE — KILLED** |
| **step701** | **Parallel routing branches** — **DONE: ALL VARIANTS HURT.** Ref=91.67%. Best: D_par3_k3=84.64% (−7.03pp); B_par3_k1 collapsed to 57.20% (−34.47pp); branch weights stayed uniform (learned α did not differentiate). **CONFIRMED: Sequential K_iter has non-trivial representational power beyond receptive field growth.** | K≤2 outer with parallel ≥ K=5 sequential | ✅ | **DONE — KILLED** |
| **step702** | **Latency-aware K_iter × K_hh frontier** — **DONE. Ref (ki5_kh2)=92.64% @121ms. ki4_kh2=91.06% @97ms (1.24×). ki3_kh2=88.88% @75ms (1.61×). ki2_kh2=85.25% @52ms (2.32×). K_hh=4 at K_iter=3/4 no improvement. Pareto: K_iter=5 dominates for quality; K_iter=3 is best tradeoff at 1.6×.** | 2x speedup at ≥95% accuracy retention | ✅ | **DONE** |
| **step703** | **Per-sample adaptive K_iter at inference** — **DONE: KILLED. All thresholds catastrophic (tau_99: −81.55pp, tau_97: −80.78pp). Logit cosine_sim is NOT a reliable convergence proxy.** Samples exit at k=2-3 under any τ<1.0, collapsing to chance. **P-SPEED fully exhausted.** Only viable path: step530 Triton kernel. | Mean eff K_iter ~3 at ≤0.5pp accuracy loss | ✅ | **DONE — KILLED** |
...
Both architectural parallelization paths failed at N=2048 D=16. Sequential K_iter is load-bearing — not just for receptive field growth but for **iterative nonlinearity stacking** (each iter applies ReLU-θ + F.normalize + reflection + AH suppression). Multi-hop precompute collapses 5 nonlinear transforms into 1, losing representational power. Parallel branches suffer the same: fewer outer iters = fewer nonlinear refinement steps.

**Implication for speed:** The only viable paths to reduce K_iter latency are:
1. **Kernel-level optimization** (P-CUDA: torch.compile, CUDA graphs, RCM reorder, Triton fused kernel) — reduce per-iter overhead without changing K_iter count
2. **Per-sample adaptive K_iter at inference** (step703) — keep training at K=5, terminate early at inference when logits stabilize
3. **Accept K_iter=5 as fundamental** and report it as an architectural constant in the paper
...
| **step223** | CIFAR-10 cross-dataset generalization — **DONE: DESIGN FLAW. 60.17% (VGG16 ImageNet features degenerate on raw 32×32 CIFAR-10). Replaced by step400 (raw pixel input, N_IN=3072).** | N=2048 | 0.98M | ✅ | **DONE — DESIGN FLAW** |
| **step224** | Scored dynamic topology — ALL NEUTRAL ±0.5pp. No benefit from per-epoch edge replacement. | N=2048 | 0.98M | ✅ | **DONE** |
| **step225** | Equilibrium propagation pilot — **DONE: KILLED. EP β=0.1: 10.06%, EP β=0.5: 10.04% vs Ref=91.67%. Chance-level performance. EP fundamentally incompatible with SGNNET's L2-normalized routing.** | N=2048 | 0.98M | ✅ | **DONE — KILLED** |
