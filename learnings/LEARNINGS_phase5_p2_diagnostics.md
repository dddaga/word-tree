...
Investigation of 26.52% → ~20% regression after trainer refactor.
Covers automated listener experiments, infrastructure bugs, root cause.
...
- Both collapsed near-random by ep120. Best was ep60-80. cosine wins by 0.28% (noise margin).
- **Decision:** cosine LR schedule going forward.
- Regression from iter1 26.52% significant — under investigation.

### Routing dropout ablation (train_routing_dropout.py, 120ep, MPS)
Results (dynamic_z_geo + thresh=0.3, N=512, D=4, aug data):
- p=0.0: 18.68%  (baseline, confirms aug baseline regression real)
- p=0.1: 18.75%  (+0.08% — below 0.5% threshold, not meaningful)
- p=0.2: 11.13%  (−7.54%, signal destroyed)
- p=0.3:  9.86%  (−8.82%, complete collapse)
- **Decision:** routing_dropout_p=0.0 going forward. Dropout not beneficial here.

### 200ep cosine baseline (CPU, DONE)
- top1_best=20.36%  best_ep=58  top1_last=17.17%
- **Decision: top1_best=20.36% < 22% threshold.**
  Regression in model config, NOT epoch count. 120ep stays standard.
...
**Definitive: architecture irrelevant at this point.**
Geo bias, threshold, routing mode — all produce identical results.
Identical best_ep=60 across all configs confirms not architecture issue.
...
Regression NOT from aug data. Something changed in training infrastructure.

---

## Root Cause: Gradient Clipping Bug in Trainer Refactor

**Bug:** `clip_grad_norm_(self.model.parameters(), 1.0)` clips ALL model
parameters. But `optimizer.zero_grad()` only zeros gradients for W_pos (only
param in optimizer). theta (512 params) and W_phase (2048 params) NOT
in optimizer — gradients ACCUMULATE across all batches without zeroing.

After K batches, theta.grad and W_phase.grad have K× per-batch gradient magnitude.
Clip norm dominated by accumulated norm, clipping W_pos gradient to
near-zero. Learning effectively stops after first few batches. Why ALL
configs/data sources gave identical ~18-19% — model barely learns W_pos.
...
### Bugfix verification (2026-03-29 05:00 PDT)
...
for actual root cause discovered via Step 6-8.
...
During bugfix verification, `dataset.py` updated from lazy per-batch
loading to in-memory loading. New code rsynced to Mac Studio mid-run.
On macOS, DataLoader workers use `spawn` (not fork) — new worker processes reimport
`dataset.py`, get NEW class definition. Workers unpickling OLD H5Dataset
instances call `__getitem__` using new code:
```
AttributeError: 'H5Dataset' object has no attribute 'features'
```

**Fix:** `make_loaders` now defaults to `num_workers=0`. All data in RAM —
no I/O to parallelise, worker processes unnecessary. No pickling, no mismatch.
