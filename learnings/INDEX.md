...
|---|---|---|---|
| [[antihebbian]] | [concepts/antihebbian.md](concepts/antihebbian.md) | Confirmed winner | alpha=1.0 alone = Gen4 base; AH = anti-collapse (eff_rank confirmed step155) |
...
| [[group_topology]] | [concepts/group_topology.md](concepts/group_topology.md) | +3.01pp N=1024, NULL N=4096 | Bypasses N^2 routing via G^2 decisions; doesn't scale |
...
| `LEARNINGS_phase5_p15_post_wave1.md` | Post-wave-1 INDEX | TOC for p15a–p15e split |
...
| K_iter optimal N-dependent | step68 (N=1024→16), step71 (N=4096→12) | [[k_iter]] |
| N-scaling non-monotonic above N=4096 | step56 (N=10000 regresses) | [[n_scaling]] |
| Turing contribution N-dependent | +1.68pp at N=1024, -0.12pp at N=4096 | [[n_scaling]] |
...
| AH = anti-collapse mechanism | step155: AH=0 → eff_rank collapse; AH=1.0 → eff_rank rises | [[antihebbian]] |
...
| AH = PREREQUISITE not regularizer | Without AH: 91.5% → 18.8% collapse (step218) — 73pp drop | [[antihebbian]] |
...
| Polarizer routing first dynamic win | +1.91pp over-polarizer α=1.5, monotonic alpha trend (step217b) | — |
| Skip connections actively rejected | Network learns skip gate=0.0 (step226) | — |
| D=16 ceiling confirmed 97.17% | N=4096 and N=8192 both converge to 97.17% (step205/209) | [[n_scaling]] |
| ΔW projection specifically load-bearing | random direction = chance (step708); receiver-only = weak; ΔW=W[recv]−W[send] = +2.11pp | [[delta_w]] |
| ΔW proj headroom non-monotone, peak N=128 | N=64(+17pp) < N=128(+24pp) > N=256(+20pp) > N=512(+12) > N=1024(+4.7) > N=2048(+1.6); HURTS at N=4096 | [[delta_w]] |
| ΔW proj vs rot crossover N=1024→2048 | proj dominates ≤1024; tie at 2048; rot marginal at 4096. Proj 3× cheaper (~2D vs ~6D MACs/edge) | [[delta_w]] |
| Sequential K_iter load-bearing (not unrollable) | multi-hop precompute (step700) and parallel branches (step701) both killed (−3 to −34pp) | [[k_iter]] |
| W_pos learned geometry = primary accuracy source | random W_pos (step401) = 10.04% chance; learned = 95.52%. Delta = 81pp | [[antihebbian]] |
| K_iter annealing/distillation/warm-transfer all KILLED at efficiency config | steps 610/611/612 — K=5 sequential passes architecturally necessary | [[k_iter]] |
| SGNNET at 0.75% of VGG16 FC true MACs (ncu-validated) | step800 — paper must use "message-passing MACs" (0.98M) vs "total MACs" (1.85M); VGG FC = 247M | — |
...
| Augmentation scale-invariant (+0.4-0.8pp all N) | step269/273/276/280 T2 curve; aug delta doesn't compress at larger N | — |
...
| Deep supervision on K_iter routing KILLED (-5 to -6.5pp) | step521 — all 4 DS variants destroy representation-building dynamics; single final-loss correct training signal | — |
...
| Audio cross-modal fails (ESC-50: -14pp vs Linear) | step406 — Whisper-tiny 384-dim features lack structure for graph routing | — |
...
| RMSNorm / pre_route normalize | −11.49pp and −6.50pp vs L2 sphere; LayerNorm wins | step116 |
...
| Per-sample adaptive K_iter at inference | All thresholds catastrophic (−80pp); logit cosine_sim NOT convergence proxy | step703 |
...
| K_iter annealing at efficiency config | All 5 schedules hurt −0.84 to −6.50pp; K=5 load-bearing at N=2048 | step610 |
...
| RCM index reordering on CUDA | 0.993× speedup = no-op; only helps when memory-BW-bound (compute-bound post-compile) | step520 |
...
**Priority shift 2026-04-14:** User directed paper 1 soft-conclusion takes priority. Exploration paused; paper-blocking experiments preferred.

1. **PAPER 1 PACKAGING (top priority)**: manuscript soft-conclusion. Framing: "SGNNET as general-purpose classification head replacing FC layers for pre-trained feature extractors, validated across vision AND language." See `paper/MANUSCRIPT_DRAFT.md`, `paper/claims.md`.
...
6. **K_iter reduction (CLOSED)**: all paths killed (annealing, warm-transfer, distillation, multi-hop, parallel branches, adaptive inference). K=5 architecturally necessary at efficiency config.
