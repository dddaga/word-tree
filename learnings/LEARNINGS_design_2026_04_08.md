...
**Hypothesis 1 — Coarse group state (S_g = mean(Z[h])) too noisy:**
Training start, W_pos random → all groups produce similar mean vectors → softmax collapses to uniform. Router gets no gradient signal early.

**Hypothesis 2 — Temporal mismatch between AH and router:**
AH: epoch-timescale (cumulative weight updates). Dynamic routing: batch-timescale (per-forward-pass). Conflict: router adapts to instantaneous batch stats while AH slowly shapes W_pos manifold.

**Hypothesis 3 — Softmax collapse at group granularity:**
step73/75 used per-neuron redistribution at K_hh=6 level (routing among 6 neighbors). step83 used per-group routing at G=16 level. Group mean destroys within-group activation diversity.

**Evidence from results:**
- C (β=0.1) = only −0.61pp — interference scales with β (supports co-adaptation)
- A (every step, −6pp) >> B (final step only, −2.65pp) — more corruption opportunities for AH signal

### Four New Experiment Candidates

**Candidate 1 — Delayed routing activation**
- Freeze router first N_freeze=30 epochs, let AH establish W_pos structure
- Unfreeze for remaining 45 epochs
- Config: step83 Config B (final-step only) + delayed activation

**Candidate 2 — Stop-gradient on group state Z**
- `S_g = mean(Z[h]).detach()` — group states as fixed inputs to router
...
- Pushes softmax away from uniform early training
...
All redistribution routing experiments (step73, 75, 76, 82, 83) at N=1024. At N=4096, AH achieves 95.87%. Open questions:
...
Static conn_hh built once at init from random W_pos. W_pos learns during training (AH gradient shapes it), optimal connectivity changes but conn_hh never follows. Rebuilding conn_hh from trained W_pos periodically could improve accuracy and enable FLOPs reduction.
...
- Positions are W_pos (already learnable); no new parameters
...
| Periodic conn_hh rebuild | step81 Hebbian rewiring (+1.12pp) — heuristic, not gradient-driven |
...
2. **Differentiability**: conn_hh rebuild detached (not differentiable). W_pos learns from AH + routing gradients.
...
**Key comparisons**:
- A vs Ref: topology plasticity alone help?
