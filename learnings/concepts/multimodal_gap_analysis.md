...
**Status:** Living doc. Updated 2026-04-20 (session 28).

## Summary

SGNNET works on Imagenette (VGG16, 10 classes) but fails on ESC-50 (Whisper, 50 classes) and CIFAR-100 (VGG16, 100 classes). Three failure modes:
...
Best: `D16_Kin15_Khh2_Ki7_N2048` = 41.5%
...
**Interpretation:** Per-node cosine sim NOT right metric. Wrong analysis.

## Activation Postmortem (step943): Correct Metrics

Postmortem on **untrained Imagenette model** (step943 baseline, session 28):
...
**CKA(seed, final) = 0.686** — routing substantially transforms representation (not identity!)

### Critical Insights

1. **Routing DOES transform Z** — CKA=0.686 (not 1.0). Cosine EDA measured per-node similarity, not pooled representation structure.

2. **Seeding alone has class signal** — probe=56.25% with untrained random weights. VGG16 features already class-separable. Random `K_in` scatter projects into N=2048 node activations preserving linear separability.

3. **One routing step: probe 56% → 67%** even without training. Routing does useful feature mixing before any learning.

4. **Participation ratio: seed 1.0 → final 3.7** — routing EXPANDS representation into more dimensions. Mechanism: distributes class signal across more orthogonal dims, easing linear readout.

5. **Fisher ratio NOT monotonic** — routing reorganizes variance in complex ways, but linear separability (probe) is right target metric.
...
- Whisper produces dense 384-dim embeddings. Top-10 PCs = 82.9% variance (effective dim ~30-50).
- `K_in=25` scatter onto N=2048 nodes: each node sees 25/384 = 6.5% of features.
- Unlike VGG features (sparse spatial), Whisper embeddings are holographic — class info distributed across ALL 384 dims. Dropping 93.5% destroys hologram.
- VGG features: localized spatial patterns. `K_in=25` scatter still captures meaningful spatial features.
- Only 40 samples/class in train — insufficient for class-conditional scatter patterns.

**Physical process insight:**
Whisper already found compact acoustic manifold (latent dim ~30-50). SGNNET random scatter destroys this manifold by projecting onto incompatible random basis.

**Fix hypothesis:**
- Smaller N (N≤256) with high `K_in` coverage (`K_in=192`, 50% coverage). Fewer nodes → each aggregates more features → hologram preserved.
...
**Fix hypotheses:**
1. D=32 or D=64 (more readout capacity)
...
| step945 | ESC-50 `K_in` coverage sweep at N=256: `K_in=32/64/128/192` | What % coverage needed? | mini_cpu T0 |
