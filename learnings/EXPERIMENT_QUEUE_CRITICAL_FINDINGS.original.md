...
**Source:** Extracted from EXPERIMENT_QUEUE.md. This file contains all confirmed findings and dead ends.
...
| **⭐ step56 N=4096 = 84.36%** | Full data 150ep, Gen4 params, best_ep=146/150, params=529K. N-scaling law: 512→69.58%, 1024→80.92%, 2048→81.10%, 4096→84.36% (PEAK), 10000→82.37% (REGRESSES). N-scaling is NOT monotonic above N=4096. N=10000 worse by 2pp. ⚠️ BUGGY ARCH — all numbers invalid, re-established by step71/step80. |
...
| **step60 KILLED: phase-distance routing (all 13 configs)** | Ref=73.55%, all 12 other configs gate-dead 10-19%. Comprehensive closure on phase-routing modifications. |
| **step54 KILLED: LR schedule sweep** | Plateau=70.78% (Ref), CosineWarmRestart T_0={10,25,50}=66-68%. Cosine warm-restarts hurt. Plateau is optimal. |
| **⭐ K_iter=16 + AH=1.0 = 74.14% (+0.61pp vs K_iter=8)** | step68 COMPLETE: K_iter=16 is Gen4 optimal at N=1024. Non-monotone: 8(73.63%)>10(73.58%)>12(72.94%)<16(74.14%)>24(70.06%). K_iter=24 cliffs −3.57pp. |
| **⭐⭐ step69 PATCH = +9.83pp — NEW REF_BASELINE_v2 = 83.36%** | step69 Ref (patched arch: input_coverage + alpha_reflect fix) = **83.36%** at 50%/75ep vs old REF_BASELINE 73.53%. All wave-1 mechanism comparisons (steps 58-68) were against a BUGGY baseline. Config A (turing=0.3) = 85.04% (+1.68pp vs Ref) at N=1024. |
| **⭐⭐⭐ step70 PROJECT BEST = 97.38%** | step70 Config B (turing=0.0, reflect=0.5, AH=1.0, N=4096, patched arch, 150ep): **97.38%** at best_ep=142. Config Ref (turing=0.3) = 97.20%. **turing=0.0 beats turing=0.3 by +0.18pp at N=4096.** Turing mechanism is slightly harmful at full scale. Contrast with N=1024 where turing=0.3 gave +1.68pp — turing contribution is N-dependent. New Gen4+ optimal at N=4096: turing=0.0, reflect=0.5, AH=1.0. |
| **⭐ step86 K_hh=4 is NEW DEFAULT** | Mac Studio (Ref/A/B/C/D) done. Mac Mini (F/G/H/I) pending resume. **Key: K_hh=4 beats K_hh=6 Ref by +0.56pp AND saves −18% FLOPs (38.8M vs 47.2M).** AH suppression nullifies local K_local edges → all K_hh reductions are free lunch. K_iter=6 costs −2.2pp (D=93.83% vs Ref=96.03%). K_hh lever >> K_iter lever at matched FLOPs. n_groups=512 (N//8) critical — n_groups=128 (N//32) causes −4.28pp regression. |
...
| **FP16 AMP confirmed default** | use_amp=True already enabled in Trainer by default — all experiments run in FP16 AMP. |
...
| **⭐ step71 K_iter=12 WINS at N=4096: 96.66%** | 5 configs complete (50%/75ep). Non-monotone: K_iter=4(92.82%) < 6(95.11%) < 8(95.87%) < 16(96.31%) < **12(96.66%)**. K_iter=12 is +0.79pp over Ref. K_iter optimal is N-dependent. Adopt K_iter=12 for future N=4096 experiments. |
...
| D=128 encoding | All LR/schedule combos stuck at ~10% | step33 |
...
For full ARM 1-4 experiment designs (step22b, step29c, step32, step36-53 etc.),
see the complete EXPERIMENT_QUEUE.md content archived below line 160 in the original
(or refer to LEARNINGS_phase5_p8_arm1_arm2.md and LEARNINGS_phase5_p9_arm3_arm5.md).
