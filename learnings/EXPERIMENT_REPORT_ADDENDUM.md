...
1. **AntiHebbian alpha=1.0 alone** = foundation. Every winning config uses it.
2. **N=4096** current sweet spot — all top results at this scale.
3. **turing=0.0** optimal at N=4096 — saves compute AND improves accuracy.
4. **K_iter=12** at N=4096 gives best accuracy per compute among ablation runs.
5. **Redistribution routing** (softmax, temperature) only viable dynamic routing paradigm.
6. **Random-group topology** (n_groups=8) improves over spatial by +3pp at N=1024.
7. **K_hh=4** new default — free lunch (+0.56pp, −18% FLOPs) confirmed by step86.
...
### What to avoid
- Per-step multiplicative gate (gate-death theorem)
...
- N > 10000 without understanding regression mechanism