...
**Why Phase 4 phasors were dropped**: Failure confounds at Phase 4 (N=256, K_iter=2, binary C,
broken cdist proximity). NOT a fundamental phasor failure. Phasor concept never tested at
Phase 5 scale (D=64, N=1024, K_iter=8, small-world). Candidate for future experiment.
...
**Why different from step29c failure**: Dynamic gate is conditioned on current Z, not just structural
W_phase. After AntiHebb training, W_pos-similar neurons have diverse Z → low res_gate for
AntiHebb-suppressed pairs. Mechanisms on orthogonal axes.
...
For fixed source A ranking targets {B_i}: Z_A terms are constant. Ranking is determined by
`Z_Bi[0] × phase_dot(A, Bi)`. Monotonic scale transformation is identical across all pairs —
top-K ordering preserved regardless of phase scale. No normalization needed.

### Phase Range: [-π, π] Canonical Implementation

[0, 2π] creates a positive bias (≈622 baseline for 63 channels). [-π, π] gives zero mean —
uncorrelated neurons score ≈0, aligned score positive, anti-aligned score negative.
