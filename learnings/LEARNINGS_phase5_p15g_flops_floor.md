...
Tracks FLOPs floor search: minimum FLOPs to hit ≥95%.

Key references entering:
- step175 (N=1024 D=32 K_hh=4 Tier-2): 94.62% @ ~3.1M — below 95%
- step177 (N=1024 D=48 K_hh=4 Tier-2, running ep110=93.96%): ~4.65M — likely below 95%
- step176-A (N=2048 D=32 K_hh=4 Tier-2): 96.18% @ ~6.1M — phase exit ✓
- step174 (N=2048 D=16 K_hh=8 Tier-2): 95.82% @ ~6.2M — phase exit ✓

Gap: N=1024 tops out ~94.6% even at full data 150ep. N=2048 right scale.
Question: at N=2048, minimum D for ≥95%?
...
**Key finding (CONFIRMED):** At N=2048 D=32 scratch, α=1.0 (not 1.05) optimal.
...
New min-FLOPs record — 24% fewer FLOPs than previous (step174/176-A at ~6.1-6.2M).

Ref chain: step172-B Tier-1 (50%/75ep) = 94.01% → full data 150ep adds **+1.12pp** → phase exit.
Prediction ~94.0-94.5%. Actual higher — warm+W_proj Tier-2 benefits from late-training convergence (best at ep133, not ep74).

**Key implication:** N=1024 D=48 warm+W_proj IS viable for phase exit at ~4.72M FLOPs.
Earlier ceiling (~94.0%) was Tier-1 artifact, not true capacity ceiling.
...
can extrapolate from N=2048 D=32 (96.18%), FLOPs floor for ≥95% may be ~4.7-5M.
...
If Tier-1 ≥93%: strong Tier-2 candidate (likely ≥95% at full data 150ep).
...
If D=24 fails 95% but D=28 passes → FLOPs floor ~5.5M, not 4.7M.

Config: scratch α=1.0, 50%/75ep (Tier-1)
FLOPs: 3×2048×4×28×8 = 5,505,024 ≈ **5.51M** (11% below 6.18M budget)
Session: step179_mm (Mac Mini CPU)

Steps 178+179 bracket-search FLOPs floor:
...
**Current floor: ~4.72M FLOPs** (step177). Steps 178/180 probe whether floor can drop below 4M at N=2048.
...
Tier-1 ref (step180): 94.14%@ep64 → Tier-2 gain: **+1.89pp**. Higher than typical +1.1-1.25pp.
Training oscillated: peaked 94.80%@ep40 → dipped 93.37%@ep80 → recovered 95.11%@ep90 → final 96.03%@ep147.
High oscillation characteristic of N=2048 — always run to completion, never kill early.
...
Actual Tier-2 (step185) gain even higher (+1.91pp).
...
Beats step181 (96.03% @ 3.93M): floor dropped 3.93M → 3.15M FLOPs.
Tier-1 ref: 93.96% → Tier-2 gain: **+1.91pp** (above typical — N=2048 long-run benefit larger than projected).

Key: accuracy only drops ~0.16pp from D=20→D=16 at Tier-2 (96.03% → 95.87%). Very flat curve here.

Mac Studio MPS freed → launched step187 (D=8 Tier-1, ~1.57M).
...
Extreme probe: Fourier sphere S^7 very low-dimensional.
...
D=12 → D=16: drop ~0.86pp at Tier-1. Following accelerating Tier-2 lift trend (D=20→+1.89pp, D=16→+1.91pp), optimistic projection: 93.10% + 1.91pp = 95.01% — barely exits.
...
D=8 confirmed floor for N=2048 scratch at 50%/75ep.
...
Tier-1→Tier-2 gain: 93.10% → 94.62% = +1.52pp (lower than D=16/D=20 pattern of +1.89-1.91pp).
+1.9pp lift does NOT hold at D=12 — lift itself diminishes with D.

→ Launched step191 (D=16 K_hh=3, same 2.36M FLOPs, Tier-1): tests if D=16 with K_hh=3 beats D=12 K_hh=4 at same budget.

## step189/190/191 — New axis probes

- **step189** (D=10 Tier-1): 92.25%@ep73 @ ~1.97M. Tier-2 ruled out (max ~94.2% even with +2.0pp). D=10 not viable.
...
**Confirmed (2026-04-10):** At fixed FLOPs, higher D + lower K_hh always dominates lower D + higher K_hh.
...
Implication: Fourier hypersphere dimension (D) determines representational capacity. K_hh is routing mechanism reducible without destroying capacity. Architecture benefits from maintaining higher D over K_hh density.
...
- **step193** (D=16 K_hh=2 Tier-2): ep20=93.04% @ ~1.57M — already near D=12 Tier-1 final (93.10%@ep75) with 130ep left. 1.57M phase exit very possible.
