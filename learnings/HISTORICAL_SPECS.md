...
Consolidated from deprecated GSD framework files. Authoritative record — data pipeline, baseline methodology, pre-Phase-5 architecture decisions.
...
### Dataset — Imagenette
- Train: 9469, Val: 3925 (13394 total)
- HDF5 store: `data/store.h5` — shape [N, 25088] float32 (VGG16 pool5 features)
- Soft labels: T=1 softmax at storage, no temperature scaling
...
- 10 Imagenette classes remapped to canonical 0-9

### UAT Results (7/7 pass)
1. Python environment — all packages import, MPS available: **PASS**
2. Imagenette on disk — 9469 train / 3925 val, 10 class folders: **PASS**
3. ImagenetteDataset loading — 9469 samples, `ds[0]` = (tensor[3,224,224], int): **PASS**
4. VGG16 feature extraction — [9469,25088] train + [3925,25088] val float32: **PASS**
...
### Methodology
- FLOPs: `thop.profile(model, inputs=...)` — returns MACs; documented as "FLOPs" (MACs × 2 convention, noted in JSON as `flops_note`)
- `count_flops(model, input_shape)` in `src/utils/metrics.py` wraps `thop.profile`
- Soft label quality thresholds (warn-only): accuracy match ≤0.1%, mean entropy ≥0.01 nats, all classes ≥5% of val
- Metrics API: `compute_all_metrics(scores, labels)` — `scores` = softmax probs (float32, [N,10])
...
- **N_in adapters**: Linear(25088→N) before SGNNET. Rejected — spatial encoding preserves structure adapter destroys. Adds params without benefit.
- **Input neuron recurrence**: Inputs in K-iteration loop. Rejected — seeding-only sufficient; recurrence introduces instability.
- **Multiple output timing**: Outputs active across all K iterations. Rejected — one-shot readout at final K_iter cleanest (no readout leak).
- **Output neurons in C_hh routing**: Rejected — pure sinks only; participating causes readout leak.
- **Soft gate (fully differentiable)**: Gaussian-only without hard gate. Deferred; hard gate + load balance is spec.
- **K-means init for W_pos**: K-means on seeded activations. Deferred; random uniform used.
...
- **KL divergence** (not MSE) for distillation loss
...
- **Wavelength derivation (D-08)**: λ = r*/2 where r* = median inter-neuron distance. One oscillation per active zone [r*/2, r*]; single inhibitory ring at d=3r*/4. All constants from N and D via r*; no free wavelength hyperparameter.
- **Masked normalization (D-07)**: Active criterion |Z_j| > ε; below-threshold neurons contribute zero to mean/var. Phasor magnitude: `||Z_j|| = sqrt(sum_d(Z_re_jd² + Z_im_jd²))`.
- **FP16 on MPS**: `torch.autocast('mps', dtype=torch.float16)` + `torch.amp.GradScaler('mps')` (requires PyTorch ≥2.3). Do NOT use `torch.cuda.amp.*` (CUDA-only). If PyTorch <2.3: autocast alone (GradScaler had MPS inf-detection bugs before 2.3).
- **W_phase (Exp 2 / Stage C only)**: ∈ ℝ^D per neuron, shape [N_hidden + N_out, D=4]; separate lr_Wphase from GA search; separate from W_pos.
- **Phasor amplitude-only routing broken**: Z ∈ ℂ^D amplitude routing failed — phase info must propagate.
...
| Stage C (spatial + W_phase Exp2) | K=2, N_hidden=256, lr=0.01, lr_Wphase=0.01 | 10.52% | W_phase trained (norm=13.36) but no accuracy improvement |
...
- NaN/inf → score = −1e6 (disqualified)
...
### Phase 4 Conclusion
Binary C mask ceiling at ~3.2 KL plateau. Static wiring hits expressiveness wall; dynamic routing required → Phase 5 redesign with learned C values + Fourier D encoding.
...
- Signed coupling (Z @ Z^T): cos-sim on S^63 ≈ noise; 5 experiments confirm ≤32%
...
- Soft beam routing: gradients through hard selection not bottleneck
...
- SSH alias: `mac-studio`; tmux at `/opt/homebrew/bin/tmux` (NOT in default PATH)
...
- 256GB RAM total — tight with 2× VGG16 + 2× dataset (~2-3GB per N=512 process)
