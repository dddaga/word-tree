...
| **step899** | Ephemeral teleportation T0. Zero extra params. Per K_iter step, re-sample K_ep=1 random long-range connection per neuron (no learned gate — avoids FM7/FM8/FM9). Addresses K_hh=2 K_iter=5 diameter limit (max 5 hops, avg path ≈6 in N=2048 small-world). Ablation: K_local={1,2} × K_ep={0,1} × dw_local={False,True} × dw_ep={False,True}. 8 configs: Ref_dw (K_l=2, K_ep=0, ΔW), A_local1_dw (K_l=1, no ep), B_ep1_l1_nodw, C_ep1_l1_ldw, D_ep1_l1_bothdw, E_ep1_l2_nodw, F_ep1_l2_ldw, G_ep1_l2_bothdw. Advance rule: any config ≥+0.5pp vs Ref_dw → T1 (step902). | `scripts/train_step899_ephemeral_teleport_t0.py` | 5060ti_cuda | **RUNNING** |
| **step900** | D-scaling T0. Hypothesis: D=16 Fourier ceiling constrains accuracy. step863 trend: D=8→91.44%, D=12→92.53%, D=16≈93.96% — monotone +1.53pp/doubling. Tests D=24 and D=32 (with and without ΔW-proj). 5 configs: Ref_d16 (D=16, no dw), A_d24, B_d32, C_d24_dw, D_d32_dw. Advance: A or B ≥+0.5pp → T1 D-scaling (step902). C/D vs A/B → ΔW scales with D. | `scripts/train_step900_d_scaling_t0.py` | studio_cpu | **RUNNING** |
