...
- **W_pos**: spatial position → determines WHO is neighbor (topology)
...
- NOT multiplicative gate over K_iter — per-edge coefficient (like AH)
- Gate-death impossible: filter recomputed from W_phase each forward pass, not compounded

**2. Alternating Training (Block Coordinate Descent)**
- Even epochs: freeze W_phase, train W_pos → topology adjusts w/ stable filters
- Odd epochs: freeze W_pos, train W_phase → filters adjust w/ stable topology
...
Diversity penalty on `cat(W_pos, W_phase)` — repels neurons w/ similar (pos, phase).
...
- If works at D=32, can scale to D=64 later
...
**Any new mechanism using W_pos as signal source will ANTAGONIZE AH.**

AH wpos suppresses contributions from W_pos-similar neighbors. If new mechanism DEPENDS on those same neighbors' contributions (routing, gating, etc.), AH removes signal it needs → catastrophic double-sparsity.
...
**Safe mechanisms** (separate parameters from W_pos):
...
1. Separate parameter (W_phase) that AH doesn't touch, OR
2. Different FUNCTION of W_pos (distance, not cosine similarity), OR
3. Entirely different state (Z activations) for routing decisions
...
step75 Config D gave +3.98pp at N=1024 w/ redistribution routing (Σw=1).
...
W/ K_hh=4 base (new default from step86), potential combined gain.

**Script:** tweak step82 script (N=4096, K_hh=4).

### Phase Alignment as Softmax Weight

step60 failed because phase coherence used as multiplicative gate.
Redistribution fix: `w_j = softmax(coherence(Z_h,Z_j)/τ, dim=2)`.
Σw=1 over K_hh neighbors → no attenuation, gate-death proof.
Directly redeems step60 failure using redistribution principle.

**Script needed.** Distinct from step73 (uses dot-product score, not phase coherence).
...
Entire step56 scaling curve invalid (buggy code). Need N={512,1024,2048,4096,8192} at
