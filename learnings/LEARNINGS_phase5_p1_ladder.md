...
**Key finding:** Topology change per input matters more than weight modulation
on fixed topology. dynamic_z (rebuilds full graph per step) > dynamic_gate
(fixed graph, activation-gated weights).
...
geometrically close AND feature-similar communicate more strongly.
