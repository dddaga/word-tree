...
All Step 5 diagnostics used SGNNET_Resonant(dynamic_z). Tested whether regression in Resonant routing layer specifically.

Script: `scripts/train_step6_baseline_control.py` — plain SGNNET_SmallWorld, no Resonant wrapper. Exact iter1 baseline_N512 replica. MPS, plateau, 120ep, store.h5.

**Result:** baseline=19.87% (iter1 baseline_N512: 24.38%)

**CONCLUSION:** Regression in SGNNET_SmallWorld or training setup, NOT Resonant routing.
...
Iter1 rerun gives 23.18% for dynamic_z — BETTER than ~20% diagnostics (3% gap).
...
~3% gap explained by dynamic_z vs dynamic_z_geo and possibly seed=42 being suboptimal. Remaining 3.34% gap vs original iter1 (26.52%) explained by Step 8.
...
26.52% → ~20% "regression" was NOT bug or infrastructure failure. Configuration test artifact:

  - Post-iter1 diagnostics tested `dynamic_z_geo` with D=4 **linear** encoding
  - iter1 used `dynamic_z` with D=4 linear encoding
  - geo mode slightly hurts at D=4 (fewer directions on S³ for spatial biases)
  - REAL breakthrough: D=8 or D=16 Fourier encoding: 25-27%+ regardless of geo mode

**New SOTA: D=16 Fourier N=512 = 27.54% (surpasses iter1 26.52%)**

### Why D matters more than N:
  - D=16 N=512: 27.54%
  - D=8  N=1024: 26.88%
  → D=4→D=16 at same N gives +6.55%
  → Doubling N 512→1024 at D=8 gives only +1.02%

### Fourier encoding explanation:
D=4 linear: `[feature, c_norm, h_norm, w_norm]` — only 3 spatial dims, on S³
D=16 Fourier: sinusoidal encoding of (h, w, c) at multiple frequencies → near-orthogonal seeds per position. At D=16 (S¹⁵), N=512 neurons get genuinely distinct directions.
D=4 fundamentally capacity-limited on S³ (~10 distinguishable directions for 512 neurons).
...
**Geo mode adds +3.1% at D=16.** At D=4 geo barely helped (~0.3%). At D=16 (S¹⁵), neurons have rich directional structure — geo bias creates structured distance signal routing can exploit.

**D=16 N=512 (29.22%) > D=8 N=1024 (28.15%) by 1.07%.** Richer directions > more neurons.

**Decision:** `dynamic_z_geo` + D=16 Fourier = new standard.
...
**Why reversal:** At D=4 (S³), neurons tightly packed; turing and reflection add crucial signal distinguishing overcrowded directions. At D=16 (S¹⁵), each neuron has unique direction — θ-gating by dot-product similarity sufficient. Extra mechanisms add noise, not signal. Simpler routing wins in richer spaces.
...
**W_phase static (28.59%) competitive vs dynamic_z_geo (29.04%)** — resonant mode works, but static phase graph doesn't add over dynamic routing.

**W_phase STATIC graph not real question.** Missing piece: EXCITATORY radiation — dynamic connections exciting similar neurons instead of inhibiting. See `LEARNINGS_phase5_p4_d16.md` step 14 for activation-based dynamic excitation design.