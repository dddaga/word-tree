...
- Hard top-k is a wall-time liability on dense hardware: **CONFIRMED** (det_step003) — sgn_r128k32 is slower than lowrank_r256 despite half the params; the top-k sort cost > the rank-halving matmul saving. Dense low-rank is the robust drone readout; top-k only pays with a sparse kernel or when accuracy is capacity-critical.
