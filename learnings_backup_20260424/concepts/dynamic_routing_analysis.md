...
**Motivation:** FGSEGNet v2 uses per-input dynamic channel gating based on input features. Hypothesis: gating nodes based on input signal |x_sum| would let SGNNET focus on input-relevant nodes.
...
**Why this is NOT gate-death (FM1):** Gate entropy 0.4–0.7 is genuinely dynamic (H=0 → static, H=0.693=ln(2) → uniform/non-discriminative). Gates ARE firing differently for different inputs. Yet accuracy still drops.

**Root cause — information bottleneck:** SGNNET K_iter message-passing is a collective computation over ALL N nodes. Every node participates in message propagation to every other node via the K_hh graph. Zero-ing out even 25% of nodes removes paths from the K_hh graph, breaking propagation chains. Unlike CNNs where each channel is independent, SGNNET nodes are connected — removing a node removes ALL edges incident to it.

**Hard gating (top-K):** drops −13.7 to −15.9pp regardless of K (10%–75% keep rate). Non-differentiable — gradient disconnect compounds over 20ep.
