...
Any multiplicative gate `g in [0,1]` applied per routing step over `K_iter` iterations compounds to `g^K_iter` signal attenuation. At `K_iter=8`:
...
Gradient through the product: `dL/dZ_0 = (prod g_k) * dL/dZ_K`. The network learns to null out the gate (g -> 0) and route through the residual path, leaving the gated mechanism decorative or destructive.

**Structural, not a hyperparameter problem.** Any per-step multiplicative gate with g < 1 dies in training for K_iter >= 4.

## Evidence

Every wave-1 dynamic routing experiment exhibited gate-death. REF_BASELINE = 73.53% (step57, AH=1.0, 50%/75ep).
...
Observable diagnostic: safety valve loss collapses to near-zero (0.002-0.005) when gate-death occurs. Routing diversity collapses — most neurons fire similarly.

## Root Cause Analysis

### Gating vs Redistribution

Static AntiHebbian routing survives because suppression is applied **before** the gather step. `F.normalize()` at the end of each routing step restores magnitude to unit vectors. Net per-step signal is conserved — AH **redirects** signal (suppression of one neighbor = more relative weight for others after normalization).
...
Gating attenuates signal without restoration. After K_iter steps the cumulative attenuation starves both forward signal and backward gradient.

### The Mechanism in Detail

1. Modified routing introduces a multiplicative gate `g in [0,1]`
...
7. Network learns to null the gate entirely, routing only through residual path
...
Signal magnitude is redistributed, not destroyed. Gradient flows cleanly across K iterations. This is graph attention at O(N*K) cost, not O(N^2).
...
- **step75**: Input-modulated temperature routing. `temp_h = tau_0 * sigma(W_temp * Z_h)`, `w_j = softmax(score_j / temp_h)`. Hot neurons route sharply, cold neurons route diffusely. Running.
...
2. The new gate removes coherence-mismatched or phase-misaligned paths
3. What survives is near-zero signal — gradient collapses

This is the "double sparsity" pattern observed in steps 58-66. AH already solves the diversity/sparsity problem optimally. Adding any further gate re-solves a solved problem, creating redundancy and signal loss.
...
Pattern: AH partially compensates for gate damage (+24pp in step63 B vs A), but the gate still destroys the AH fixed point. The interaction is always destructive.

## Experiments That Escaped (Partially)

### step60 PhaseRouting — softmax weights, not pure gate

step60 used softmax weights (sum(w)=1) rather than a pure multiplicative gate. Signal magnitude was redistributed, not destroyed. This should have avoided gate-death.

Why it still failed:
1. **Double-sparsity**: phase routing + AH sparsification + beam left too few active paths for gradient
2. **Scale confounds**: tested at N=1024, D=64, early Gen4 (no turing/reflect), before arch bugs were fixed
...
A/B/C used soft (sigmoid) gating with residual paths, avoiding total signal collapse. Small gains (+0.87 to +1.83pp) on pre-Gen4 base. D/E used hard gates and gate-died. The soft gate partially escaped by keeping gate values near 1.0 (minimal attenuation) but this also means minimal routing effect.
...
Config F marginally escaped (+0.76pp) while B/C/D/E gate-died. F likely had the weakest gating interaction, leaving AH routing mostly undisturbed.

### step73 configs B/C/D — softmax redistribution (first clean escape)

Config D = 86.34% (+1.78pp over Ref 84.56%). First mechanism to cleanly beat static AH routing by using redistribution (sum(w)=1) instead of gating. This is the proof that the gate-death theorem's fix works.

## Implications for Future Design

### The constraint

All future dynamic routing mechanisms in SGNNET must be **conservative** (signal-preserving). Specifically:

1. **No multiplicative gates with g < 1.** Any per-step gate kills gradients over K_iter >= 4 iterations.
2. **Weights must sum to 1** (softmax or equivalent normalization). This ensures total incoming signal is conserved per routing step.
3. **Do not add conditions that activations must satisfy to propagate.** Every condition is an implicit gate.
4. **Gradient through the routing path must be O(1) per step**, not O(g^K).

### Additional structural constraints at scale

Even with redistribution (no gate-death), neuron-level dynamic routing at N=4096 is problematic:
- Each routing weight w_ij ~ 1/K where K is topK sparsity
...
**Group-level routing** (G=16 groups) reduces the decision space from N*K ~ 65536 to G^2 = 256 decisions. Group state S_g = mean(Z[h] for h in group g) provides stable gradient signal. step82 group topology = +2-3pp confirms the structural benefit. step83 inter-group routing results are mixed (A=-5.35pp, B=-2.14pp vs Ref) — the topology itself may matter more than routing between groups.

### Design checklist for new routing mechanisms

- [ ] Is the signal magnitude conserved per routing step? (sum(w) = 1 or equivalent)
- [ ] Does the mechanism interact with AH additively (not multiplicatively)?
- [ ] Is the gradient through K_iter steps bounded (no compound decay)?
- [ ] At target N, are routing weights numerically well-separated? (not 1/N)
- [ ] Does the mechanism avoid re-solving the diversity problem AH already solves?

## See Also

- [[antihebbian]] — the core diversity mechanism; all gate-death occurs relative to AH's stable fixed point
