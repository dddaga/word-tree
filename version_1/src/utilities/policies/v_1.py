...
if DB == "MONGO":
    from src.utilities.graph_operations.mongodb import (
        touch_connection_db,
        find_word,
        update_graph_context,
    )
if DB == "REDIS":
    from src.utilities.graph_operations.redis import (
        touch_connection_db,
        find_word,
        update_graph_context,
    )
...
def train_graph(context_history, target, running_context):
    ...
    for index, connection in enumerate(context_trajectory):
        ...
        weight = (
            LEANING_RATE
            * (CONTEXT_DECAY ** (connection_count - index))
            * (1 / np.sqrt(update_count + 1))
        )
    ...
    target_connection["updated_context"] = (
        target_connection["context"]
    ...
    for negative_index, negative_connction in enumerate(negative_neighbours):
        ...
        negative_connction["updated_context"] = (
            negative_connction["context"]
            - LEANING_RATE * (1 / np.sqrt(update_count + 1)) * negative_gradient
        )
