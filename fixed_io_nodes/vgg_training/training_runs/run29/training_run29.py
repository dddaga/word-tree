"""
run29 — Uniform routing ablation: replace softmax with 1/degree routing.

Hypothesis: Softmax routing concentrates gradient on high-activation nodes,
causing starvation at low cardinality and potentially limiting accuracy even
at high cardinality. Uniform routing distributes gradient equally.

Technique: Monkey-patch update_activations in native.layer's namespace
before model construction. core/ and native/ files are UNTOUCHED.

Config identical to run17 (N=4146, C=200, T=4.0, 100% data, no FFN, 40ep).
Only change: routing weights = 1/degree instead of softmax(act_strength).

Comparison:
  run17: softmax routing, C=200 → 86.85% @ep38
  run29: uniform routing, C=200 → ???
"""
import os
import sys
...
def main(config_path: str = None):
    ...
    print("Training started (run29 -- UNIFORM ROUTING, N=4146, C=200, no FFN)")
    print("  Routing: 1/degree (uniform) instead of softmax")
    print("  All else identical to run17")
