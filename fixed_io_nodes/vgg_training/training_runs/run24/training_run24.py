"""
run24 — FLOPs reduction cycle 2 (beam search), step 2: beam_width=1024.

Goal: ~N/4 sources. 2.6x FLOPs savings vs run17 baseline.

Config identical to run17 except beam_width=1024. Clean single-variable ablation.
Comparison: run17 (beam=0, 86.85%), run23 (beam=2048, 86.50% @ep40, -0.35pp).
FLOPs: ~0.37B/fwd (2.6x cheaper than run17).

Series:
  run23: beam=2048 → 86.50% @ep40 (N/2 filter, -0.35pp — lossless)
  run24: beam=1024 → this run (N/4 filter, 2.6x savings)
  run25: beam=512  → pending (N/8 filter, 3.6x savings)
  run26: beam=256  → pending (N/16 filter, 4.4x savings, cliff probe)
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run24 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run24 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run24/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run24/run24_weights.pt")
    ...
    print("Training started (run24 — beam=1024, N=4146, T=4.0, 100% data, no FFN)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run24 — beam=1024, N=4146, T=4.0, 100% data, no FFN")
