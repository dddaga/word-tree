"""
run10 — Remove FFN diagnostic.

Change from run6/training.py:
  - CustomHybridModel has no nn.Linear head.
  - output_nodes=10 in config; GNN act_strength for those 10 nodes are the class logits.
  - Hypothesis: if accuracy is similar to run6, most learning was happening in the FFN,
    not the GNN. If accuracy collapses, the GNN is contributing meaningful representations.
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run10 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run10 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run10/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run10/run10_weights.pt")
    ...
    print("Training started (run10 — no FFN head)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run10 — no FFN head")
