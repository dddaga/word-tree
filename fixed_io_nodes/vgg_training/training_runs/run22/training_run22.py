"""
run22 — FLOPs reduction cycle 1, extreme probe: cardinality=4.

Goal: 50x FLOPs reduction vs run17 — anchor the floor of the cardinality lever.

Config identical to run17 except cardinality=4. Clean single-variable ablation.
At C=4 (~0.1% density), graph is effectively a random sparse expander — aggregation
likely too weak to propagate class signals. Pairs with run19/20/21 to give full trend curve.
FLOPs: 0.02B/fwd (50x cheaper than run17).

Series:
  run19: C=100 → 80.66% @ep40 (2x FLOPs reduction)
  run20: C=50  → pending (4x FLOPs reduction)
  run21: C=25  → pending (8x FLOPs reduction, cliff probe)
  run22: C=4   → this run (50x FLOPs reduction, floor probe)
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run22 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run22 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run22/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run22/run22_weights.pt")
    ...
    print("Training started (run22 — C=4, N=4146, T=4.0, 100% data, no FFN)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run22 — C=4, N=4146, T=4.0, 100% data, no FFN")
