"""
run14 — Layered topology ablation against run11 (flat).

Change from run11 (base):
  - graph.topology = "layered" (was "flat"/default)
  - 50% training data (stratified by class) for faster iteration
  - All other config identical to run11 (N=4146, ~1K intermediates, no FFN).

Hypothesis: Layered topology (output<-intermediate only, intermediate<-any except self)
may create more structured information flow than flat random edges. Previous layered runs
(7-9) were confounded. This is the first clean ablation on a working FFN-free config.

Comparison: run11 (flat, 61.32% val @ep37). Same architecture, same seed, only topology differs.
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run14 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run14 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run14/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run14/run14_weights.pt")
    ...
    print("Training started (run14 — layered topology, no FFN head, 50% data)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run14 — no FFN head")
