"""
run11 — No FFN + fewer intermediate nodes.

Change from run10:
  - total_nodes=4146 (3136 input + 1000 intermediate + 10 output)
  - run10 had 15454 total (~12308 intermediate)
  - No nn.Linear head (same as run10) — GNN act_strength used directly as class logits.
  - Tests: does reducing intermediate nodes from ~12K to ~1K help, hurt, or not matter
    when the GNN must do classification on its own?

Context: run10 showed the GNN learns without FFN but slowly (~30% val at ep13).
All experiments from run10 onwards drop the FFN. Target: run6's 85.81% (with FFN).
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """No FFN — GNN outputs 10 class logits directly via act_strength."""
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run11 requires output_nodes=10 (no FFN), got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run11/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run11/run11_weights.pt")
    ...
    print("Training started (run11 — no FFN, reduced intermediate nodes)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run11 — no FFN, reduced intermediates")
