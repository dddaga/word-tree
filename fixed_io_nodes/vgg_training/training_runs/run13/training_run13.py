"""
run13 — Routing temperature scaling (gradient starvation fix attempt).

Change from run10 (base):
  - model.routing_temperature = 7.0 (was 1.0/default)
...
Part of temperature sweep: run10(T=1.0), run13(T=7.0), run13(T=7.0).
...
class CustomHybridModel(nn.Module):
    """
    run13 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run13 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run13/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run13/run13_weights.pt")
    ...
    print("Training started (run13 — routing temperature=7.0, no FFN head, 50% data)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run13 — no FFN head")
