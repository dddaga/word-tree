"""
run26 — FLOPs reduction cycle 2 (beam search), step 4: beam_width=256.

Goal: ~N/16 sources. 4.4x FLOPs savings vs run17 — cliff probe.

Config identical to run17 except beam_width=256. Clean single-variable ablation.
At N/16 beam, winner-take-all rich-get-richer dynamics may emerge. Anchors the floor.
FLOPs: ~0.22B/fwd (4.4x cheaper than run17).

Series:
  run23: beam=2048 → 86.50% @ep40 (N/2 filter, lossless)
  run24: beam=1024 → TBD (N/4 filter)
  run25: beam=512  → TBD (N/8 filter)
  run26: beam=256  → this run (N/16 filter, cliff probe)
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run26 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run26 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run26/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run26/run26_weights.pt")
    ...
    print("Training started (run26 — beam=256, N=4146, T=4.0, 100% data, no FFN)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run26 — beam=256, N=4146, T=4.0, 100% data, no FFN")
