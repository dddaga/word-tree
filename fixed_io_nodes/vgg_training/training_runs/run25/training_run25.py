"""
run25 — FLOPs reduction cycle 2 (beam search), step 3: beam_width=512.

Goal: ~N/8 sources. 3.6x FLOPs savings vs run17 baseline.

Config identical to run17 except beam_width=512. Clean single-variable ablation.
Comparison: run17 (beam=0, 86.85%), run23 (beam=2048, 86.50%), run24 (beam=1024, TBD).
FLOPs: ~0.27B/fwd (3.6x cheaper than run17).

Series:
  run23: beam=2048 → 86.50% @ep40 (N/2 filter, lossless)
  run24: beam=1024 → TBD (N/4 filter)
  run25: beam=512  → this run (N/8 filter, likely elbow)
  run26: beam=256  → pending (N/16 filter, cliff probe)
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run25 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run25 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run25/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run25/run25_weights.pt")
    ...
    print("Training started (run25 — beam=512, N=4146, T=4.0, 100% data, no FFN)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run25 — beam=512, N=4146, T=4.0, 100% data, no FFN")
