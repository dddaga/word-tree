"""
run30 — Uniform routing at C=2: does removing softmax fix gradient starvation?

run29 CONFIRMED: uniform routing is +2.30pp better than softmax at C=200 (89.15% vs 86.85%).
Now test at C=2 — the extreme sparsity where softmax routing fails catastrophically
(run22 C=4: 25.83%, run27 C=2: 29.04%).

If uniform routing works at C=2, this proves:
  1. Softmax was THE cause of gradient starvation at low cardinality
  2. Complex-number GNNs are viable at extreme sparsity
  3. Path to ~1M FLOPs is open

Technique: Same monkey-patch as run29. core/ and native/ UNTOUCHED.
Config: C=2, all else = run17. Uniform routing (1/degree).

Comparison:
  run27: softmax, C=2 → 29.04% @ep36 (starvation)
  run22: softmax, C=4 → 25.83% @ep40 (starvation)
  run29: uniform, C=200 → 89.15% @ep38 (NEW BEST)
  run30: uniform, C=2 → ???
"""
import os
import sys
...
def main(config_path: str = None):
    ...
    print("Training started (run30 -- UNIFORM ROUTING, N=4146, C=2, no FFN)")
    print("  Routing: 1/degree (uniform) instead of softmax")
    print("  C=2 extreme sparsity — testing if uniform routing fixes starvation")
