"""
run16 — Combine run11 (N=4146) + run12 (T=4.0), full data.

Changes from run12 (base):
  - graph.total_nodes = 4146 (was 15454) — same as run11 (3136 input + 1000 intermediate + 10 output)
  - data_fraction = 1.0 (100% data) — faster epochs with fewer nodes, 66K params benefits from more samples

Hypothesis: run11's N=4146 and run12's T=4.0 are independent improvements that should compound.
  run11: N=4146, T=1.0 → 61.32%
  run12: N=15454, T=4.0 → 82.19%
  run16: N=4146, T=4.0, 100% data → expected best FFN-free result so far.

Comparison targets:
  run12 (T=4.0, N=15454, 50% data, 82.19%) — isolates effect of N=4146 + full data
  run11 (T=1.0, N=4146, 100% data, 61.32%) — isolates effect of T=4.0
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run16 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run16 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run16/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run16/run16_weights.pt")
    ...
    print("Training started (run16 — N=4146, T=4.0, 100% data, no FFN head)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run16 — N=4146, T=4.0, 100% data, no FFN")
