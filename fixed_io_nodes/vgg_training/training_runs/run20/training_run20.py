"""
run20 — FLOPs reduction cycle 1, step 2: cardinality=50.

Goal: 4x FLOPs reduction vs run17 (C=200 → C=50) with minimal accuracy loss.

Config identical to run17 except cardinality=50. Clean single-variable ablation.
Comparison: run17 (C=200, 86.85% @ep38, 0.96B FLOPs), run19 (C=100, 80.66% @ep40, 0.48B FLOPs).
FLOPs: 0.24B/fwd (4x cheaper than run17).

Series:
  run19: C=100 → 80.66% @ep40 (2x FLOPs reduction)
  run20: C=50  → this run (4x FLOPs reduction)
  run21: C=25  → pending (8x FLOPs reduction, cliff probe)
  run22: C=4   → pending (50x FLOPs reduction, floor probe)
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run20 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run20 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run20/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run20/run20_weights.pt")
    ...
    print("Training started (run20 — C=50, N=4146, T=4.0, 100% data, no FFN)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run20 — C=50, N=4146, T=4.0, 100% data, no FFN")
