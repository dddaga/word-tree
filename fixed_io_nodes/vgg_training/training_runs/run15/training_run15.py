"""
run15 — Temperature annealing: T=7.0 → 1.0 over all training steps.

Change from run13 (base):
  - routing_temperature decays linearly from 7.0 to 1.0 over all steps.
    At every step: T = 7.0 - 6.0 * (global_step / total_steps)
    Set via model.gnn._routing_temperature before each forward pass.
  - All other config identical to run13 (N=15454, no FFN, 50% data).

Hypothesis: Starting with near-uniform routing (T=7.0) spreads gradient broadly in
early training (letting all nodes develop), then sharpening to T=1.0 allows the network
to specialize routing pathways. Combines benefits of both extremes rather than picking one.

Temperature sweep context:
  run10: T=1.0 fixed → 40.33%
  run12: T=4.0 fixed → 82.19%
  run13: T=7.0 fixed → pending
  run15: T=7.0→1.0 annealed → this run
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run15 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run15 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    # Temperature annealing schedule: linear decay from temp_anneal_start to temp_anneal_end
    temp_start = cfg.get("model", {}).get("temp_anneal_start", 7.0)
    temp_end = cfg.get("model", {}).get("temp_anneal_end", 1.0)

    # Resolve paths relative to vgg_training/ (parent of training_runs/)
    vgg_dir = THIS_DIR.parents[1]

    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run15/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run15/run15_weights.pt")
    ...
    print(f"Training started (run15 — temperature annealing {temp_start}→{temp_end}, no FFN head, 50% data)")
    for epoch in range(start_epoch, epochs):
        ...
        for x, y in pbar:
            ...
            # Linear temperature decay: T=temp_start at step 0, T=temp_end at last step
            current_temp = temp_start - (temp_start - temp_end) * (global_step / total_steps)
            model.gnn._routing_temperature = current_temp
            ...
            writer.add_scalar("Training/Routing_Temperature", current_temp, global_step)
            preds = logits.argmax(dim=1)
            ground_truth = y.argmax(dim=1)
            correct += (preds == ground_truth).sum().item()
            total += y.size(0)
            epoch_loss += loss.item() * y.size(0)
            pbar.set_postfix({"Loss": f"{loss.item():.4f}", "T": f"{current_temp:.2f}"})
            global_step += 1
        ...
        # Temperature at end of this epoch
        epoch_end_temp = temp_start - (temp_start - temp_end) * (global_step / total_steps)
        print(
            f"Epoch {epoch + 1} Summary: Train Loss: {avg_train_loss:.4f}, "
            f"Train Acc: {train_acc:.2f}%, "
            f"Val Acc: {val_acc:.2f}%, Val Loss: {val_loss:.4f}, "
            f"T_end: {epoch_end_temp:.2f}"
        )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run15 — temperature annealing 7→1")
