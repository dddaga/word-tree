"""
run21 — FLOPs reduction cycle 1, step 3: cardinality=25.

Goal: 8x FLOPs reduction vs run17 — probe the accuracy cliff.

Config identical to run17 except cardinality=25. Clean single-variable ablation.
Comparison: run17 (C=200, 86.85% @ep38, 0.96B FLOPs), run19 (C=100, 80.66%), run20 (C=50, TBD).
FLOPs: 0.12B/fwd (8x cheaper than run17).

Series:
  run19: C=100 → 80.66% @ep40 (2x FLOPs reduction)
  run20: C=50  → pending (4x FLOPs reduction)
  run21: C=25  → this run (8x FLOPs reduction, cliff probe)
...
class CustomHybridModel(nn.Module):
    """
    run21 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run21 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run21/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run21/run21_weights.pt")
    ...
    print("Training started (run21 — C=25, N=4146, T=4.0, 100% data, no FFN)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run21 — C=25, N=4146, T=4.0, 100% data, no FFN")
