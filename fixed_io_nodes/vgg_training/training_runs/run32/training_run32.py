"""
run32 — Uniform routing at C=100: mapping the accuracy peak vs cardinality.

Uniform routing gain is cardinality-dependent:
  C=200 (run29): +2.30pp  → 89.15%
  C=50  (run31): +7.90pp  → 79.54%
  C=2   (run30): -10.65pp → 18.39%
Peak gain at moderate C. run32 tests C=100 (softmax baseline: run19 = 80.66%).
Expected ~86% if uniform gain scales like C=50.
...
def main(config_path: str = None):
    ...
    print("Training started (run32 -- UNIFORM ROUTING, N=4146, C=100, no FFN)")
    print("  Routing: 1/degree (uniform) instead of softmax")
    print("  C=100 — mapping accuracy peak; 2x FLOPs savings vs run17")
