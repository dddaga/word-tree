"""
run27 — Team member replica: N=2048, D=16, C=2, K_iter=5.

Exact replication of team member config on our architecture.
Team member reported: 95.52% val, 0.98M FLOPs (different formula, counts 3 ops/edge/dim/iter).
Our formula: 5.36M FLOPs/sample, 21.4M/step (44.5x cheaper than run17).

EXPECTED: FAIL. C=2 is below our run22 floor (C=4). Our softmax routing causes
gradient starvation at low cardinality. Their simpler aggregation avoids this.
Early-stop at ep15 if val<15%.

CRITICAL: input_nodes=1568 (1568x16=25088=VGG16 features). Only 470 intermediate nodes.

Baseline: run17 (C=200, D=8, 86.85% @ep38).
Companion: run28 (C=25, D=16) -- D=16 at safe cardinality.
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run27 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run27 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run27/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run27/run27_weights.pt")
    ...
    print("Training started (run27 -- N=2048, D=16, C=2, team member replica)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run27 -- N=2048, D=16, C=2, team member replica")
