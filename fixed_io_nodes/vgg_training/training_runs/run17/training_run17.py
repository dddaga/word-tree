"""
run17 — Re-run of run16 under new post-update LayerNorm (commit f11f768).
...
Series:
  run17: N=4146, T=4.0, 100% data → primary validation at best config
...
class CustomHybridModel(nn.Module):
    """
    run17 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run17 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run17/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run17/run17_weights.pt")
    ...
    print("Training started (run17 — N=4146, T=4.0, 100% data, no FFN, post-update LN)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run17 — N=4146, T=4.0, 100% data, no FFN, post-update LN")
