"""
run28 -- D=16 at safe cardinality: N=2048, D=16, C=25.

Tests vector_dim=16 with cardinality where learning is expected (C=25 = run21 cliff level).
Companion to run27 (C=2, D=16 -- exact team member config, expected to fail).
Confounded: D=16 forces input_nodes=1568 (vs run17's 3136). Cannot isolate D alone.

In our FLOPs formula: ~120M FLOPs/step (8x cheaper than run17).
CRITICAL: input_nodes=1568 (1568x16=25088=VGG16 features). Only 470 intermediate nodes.

Baseline: run17 (C=200, D=8, 86.85% @ep38), run21 (C=25, D=8, TBD).
"""
import os
import sys
...
class CustomHybridModel(nn.Module):
    """
    run28 variant: no nn.Linear head.
    ...
    def __init__(self, cfg):
        ...
        assert self.output_nodes == 10, (
            f"run28 requires output_nodes=10, got {self.output_nodes}"
        )
...
def main(config_path: str = None):
    ...
    raw_log_dir = cfg.get("system", {}).get("tensorboard_dir", "training_runs/run28/tensorboard")
    log_dir = _resolve_path(raw_log_dir, vgg_dir)
    writer = SummaryWriter(log_dir=str(log_dir))

    raw_save_path = cfg.get("system", {}).get("weights_save_path", "training_runs/run28/run28_weights.pt")
    ...
    print("Training started (run28 -- N=2048, D=16, C=25)")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train run28 -- N=2048, D=16, C=25")
