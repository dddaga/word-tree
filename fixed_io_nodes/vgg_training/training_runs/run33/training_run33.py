"""
run33 — Uniform routing at C=200, I=3: test iterations as FLOPs lever.

run29 (uniform, I=5, C=200): 89.15% @ 0.96B FLOPs
run33 (uniform, I=3, C=200): ??? @ ~0.48B FLOPs (2 iterations instead of 4)

Hypothesis: Uniform routing converges fast (run29 hit run17's best by ep25).
May not need all 5 iterations. If accuracy >85% at I=3, iterations are a
better FLOPs lever than cardinality (preserves connectivity/expressiveness).
...
def main(config_path: str = None):
    ...
    print("Training started (run33 -- UNIFORM ROUTING, N=4146, C=200, I=3, no FFN)")
    print("  Routing: 1/degree (uniform) instead of softmax")
    print("  I=3 — testing if fewer iterations maintain accuracy at 2x FLOPs savings")
