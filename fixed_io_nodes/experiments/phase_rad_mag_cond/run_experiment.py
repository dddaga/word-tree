"""
Run experiment: phase-only radiation, magnitude-only conduction.
Uses PyTorch-only stack (no Qdrant). Run from repo root (word-tree):

  python fixed_io_nodes/experiments/phase_rad_mag_cond/run_experiment.py

Or from fixed_io_nodes with parent on path:

  python experiments/phase_rad_mag_cond/run_experiment.py
"""
...
# Ensure package is importable (repo root = parent of fixed_io_nodes)
...
from fixed_io_nodes.experiments.phase_rad_mag_cond.optim import ExperimentGNNAdam


def load_iris_dataset():
    """Iris dataset: (N, 1, 4) after arccos, labels (N,) 0/1/2."""
    iris_data = load_iris()
...
class MLPGNNModel(nn.Module):
    """MLP -> GNN (experiment layer). Same shape as USAGE.md."""

    def __init__(self, input_dim=4, input_nodes=4, output_nodes=3, vector_dim=4, lr=0.001, accumulation_steps=4, **gnn_kwargs):
        super().__init__()
        self.input_nodes = input_nodes
        self.vector_dim = vector_dim
        self.linear = nn.Linear(input_dim, input_nodes * vector_dim)
        self.tanh = nn.Tanh()
        self.gnn = create_experiment_layer(
...
def main():
    ...
    # Config matching distributed_test (PyTorch node store, no Qdrant needed at runtime)
    cfg = get_config(
        input_nodes=4,
        output_nodes=3,
        vector_dim=4,
    ...
    optimizer = ExperimentGNNAdam(model, lr=cfg["training"]["lr"], betas=(0.9, 0.999), eps=1e-8)
    dataset = load_iris_dataset()
    dataloader = DataLoader(dataset, batch_size=cfg["training"]["accumulation_steps"], shuffle=True)
