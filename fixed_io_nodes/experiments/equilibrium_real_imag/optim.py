"""
Optimizer for models that contain RealImagNeurographLayer.
Same behavior as GNNAdam: head params with Adam; GNN grads from sink -> accumulator -> step.
"""
...
from fixed_io_nodes.experiments.equilibrium_real_imag.layer import RealImagNeurographLayer


class RealImagGNNAdam(optim.Adam):
    """
    Optimizer for models that contain RealImagNeurographLayer(s).
    ...
    def __init__(
        ...
        gnn_layers: List[RealImagNeurographLayer] = [
            m for m in model.modules() if isinstance(m, RealImagNeurographLayer)
        ]
        if not gnn_layers:
            raise ValueError(
                "Model has no RealImagNeurographLayer. "
                "RealImagGNNAdam is for models with the real/imag layer."
            )
