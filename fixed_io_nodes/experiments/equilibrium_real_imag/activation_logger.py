"""Log activations per step to file for equilibrium analysis. Uses experiment utils for real/imag."""
...
class ActivationLogger:
    """Appends per (step, node_id) activations to a CSV file."""

    def __init__(self, log_path: str, vector_dim: int, gamma: float, include_phase_mag: bool = False):
    ...
    def _ensure_header(self):
        ...
        fieldnames = ["step", "node_id", "strength", "real", "imag"]
    ...
    def log_step(self, step: int, active_nodes: Dict[Any, Any], gamma: float = None):
        """Log one step: for each node write step, node_id, strength, real, imag, optionally phase/mag."""
        ...
        for node_id, node in active_nodes.items():
            ...
            if strength.dim() > 0:
                strength = strength.item() if strength.numel() == 1 else strength.sum().item()
            else:
