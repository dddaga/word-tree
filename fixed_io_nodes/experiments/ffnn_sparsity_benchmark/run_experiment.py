...
Same single-hidden-layer architecture as ffnn_wine_benchmark, but each weight
matrix is filtered by a fixed binary "conduction mask": for every non-input
node, exactly floor((1 − sparsity) × fan_in) incoming connections are kept
(chosen once at init, never changed). This is not dropout — the graph topology
is static throughout training.
...
sparsity = 0.0 → fully-dense (identical to baseline FFNN)
sparsity = 1.0 → zero active weights (only biases survive)

Outputs written to a timestamped sub-folder under:
    fixed_io_nodes/experiments/ffnn_sparsity_benchmark/runs/

Run from repo root:
    python3 fixed_io_nodes/experiments/ffnn_sparsity_benchmark/run_experiment.py
"""
...
# Hidden size used for the "fixed-h, sweep-sparsity" loss-curve sample plot
SAMPLE_HIDDEN   = 500
...
def make_conduction_mask(
    ...
    At sparsity=1.0 the mask is all-zeros (no active connections).
    Otherwise each row has at least 1 active connection.
    """
    if sparsity >= 1.0:
...
class SparseFFNN(nn.Module):
    """Single hidden layer FFNN with fixed conduction masks on both weight matrices.

    Masked weights receive zero gradient (mask=0 kills the chain rule term),
    so they never move from their initial values — no re-masking step needed.
    """
    ...
    def __init__(
        ...
        self.register_buffer(
            "mask1", make_conduction_mask(hidden_size, input_dim,   sparsity, rng)
        )
        self.register_buffer(
            "mask2", make_conduction_mask(output_dim,  hidden_size, sparsity, rng)
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = F.relu(F.linear(x, self.fc1.weight * self.mask1, self.fc1.bias))
        return F.linear(x, self.fc2.weight * self.mask2, self.fc2.bias)
...
def count_active_params(model: SparseFFNN) -> int:
    """Active weight connections + all biases."""
    w = int(model.mask1.sum().item()) + int(model.mask2.sum().item())
    b = model.fc1.bias.numel() + model.fc2.bias.numel()
    return w + b


def count_active_macs(model: SparseFFNN) -> int:
    """MACs proportional to active (unmasked) connections only."""
    return int(model.mask1.sum().item()) + int(model.mask2.sum().item())
...
def train_and_eval(
    ...
    model     = SparseFFNN(X_tr.shape[1], hidden_size, 2, sparsity).to(DEVICE)
...
_SPARSITY_PCT = [int(s * 100) for s in SPARSITY_LEVELS]   # 0, 10, …, 100
...
def plot_accuracy_vs_sparsity(df: pd.DataFrame, out_dir: Path):
    ...
    fig.suptitle(
        "Test Accuracy vs Sparsity  |  each line = one hidden width",
...
def plot_accuracy_vs_hidden(df: pd.DataFrame, out_dir: Path):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 5))
    fig.suptitle(
        "Test Accuracy vs Hidden Width  |  each line = one sparsity level",
        fontsize=12, fontweight="bold",
    )
...
def plot_metrics_dashboard(df: pd.DataFrame, out_dir: Path):
    ...
    fig.suptitle(
        "FFNN Conduction Sparsity Benchmark  |  metrics averaged over hidden widths",
...
def plot_tradeoff(df: pd.DataFrame, out_dir: Path):
    ...
    fig.suptitle("Complexity vs Accuracy Trade-off", fontsize=12, fontweight="bold")
...
def plot_sample_loss_curves(sample_curves: list, out_dir: Path):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(16, 5))
    fig.suptitle(
        f"Loss Curves  |  hidden_size={SAMPLE_HIDDEN}, varying sparsity",
        fontsize=12, fontweight="bold",
    )
...
def write_findings(df: pd.DataFrame, out_dir: Path):
    ...
    lines += [sep, "  FFNN Conduction Sparsity Benchmark — Key Findings", sep, ""]
    ...
    # For each hidden size, find sparsity where acc drops below 95% of dense
    ...
    # Efficiency score at 0% vs 50% sparsity (averaged over hidden sizes)
...
def main():
    ...
    print(
        ...
        f"{'time(s)':>9} {'acc':>8} {'f1':>8}"
    )
    print("-" * 60)

    results      = []
    sample_curves = []   # loss curves for hidden_size == SAMPLE_HIDDEN
    ...
    # pivot tables for heatmaps (rows=hidden_size, cols=sparsity)
    ...
    plot_heatmap(pivot_acc, "Accuracy",
                 "Test Accuracy  |  rows=hidden_size, cols=sparsity",
                 out_dir=_RUN_DIR, fname="heatmap_accuracy.png")
    plot_heatmap(pivot_f1,  "Weighted F1",
                 "Weighted F1    |  rows=hidden_size, cols=sparsity",
